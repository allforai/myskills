...
## Phase 4-7 预置脚本优先（自动模式）

自动模式下，Phase 4-7 优先使用预置脚本（位于 `${CLAUDE_PLUGIN_ROOT}/scripts/`）：
...
> Phase 4 (use-case)、Phase 5 (feature-gap)、Phase 6 (ui-design)
> 之间无数据依赖（均仅依赖 product-map + screen-map），使用 Agent tool 并行执行。

### 执行方式

Phase 3 checkpoint 通过后，用**单条消息发出 3 个 Agent tool 调用**并行执行。
  ...
  检查每个 Agent 的返回结果:
    ...
    部分失败 →
      ...
      询问用户:
        ...
        2. 跳过继续到 Phase 7（design-audit 对 use-case/gap 标注为可选依赖）
        3. 中止流程
    全部失败 →
      向用户报告所有错误
      询问: 全部重试 / 中止
...
~~~
resume 模式检测 Phase 4-6 完成状态:
  3 个产出全部存在（use-case, feature-gap, ui-design） → 跳过 Phase 4-6，进入 Phase 7
...
4. **轻量校验不替代终审** — 阶段间的轻量校验仅做快速检查，完整校验由 Phase 7 design-audit 负责
5. **只读不改上游** — 后序阶段发现上游问题时只报告，不自动回退修改上游产物
6. **并行 Agent 产出隔离** — Phase 4-6 的 3 个并行 Agent 各自写入独立目录和分片 pipeline-decisions 文件，不读写其他 Agent 的产出。聚合由编排器在全部完成后统一执行
