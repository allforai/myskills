...
`{kind:"supervisor", verdict:"confirmed"|"rejected", summary, rerun_exit_code, evidence: "<real captured output>", refutation?, vacuous?, reality_gated?}`.
Use `confirmed` only where the former `done:true` rule is satisfied. On `rejected`,
`refutation` says exactly what failed. Set `vacuous:true` specifically when the
