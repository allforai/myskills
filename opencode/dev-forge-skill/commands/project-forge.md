...
│    代码稳定后设计演示数据，提示执行 demo-forge design 工作流  │
│    输出: demo-plan.json                                      │
...
| 能力 | 探测方式 | 重要性 | 降级行为 |
|------|---------|--------|---------|
| Playwright | `mcp__playwright__browser_navigate` 可用性 | Phase 5-8 必需 | 阻塞验证阶段，提示安装 |
| Maestro | `which maestro` CLI 可用性（Bash 检测） | mobile-native 子项目必需 | Playwright 降级（仅测 Web 端点）|
| OpenRouter (MCP) | `mcp__ai-gateway__ask_model` 可用性 | 可选 | 跳过 XV 交叉验证 |
...
- **Playwright 未就绪**：Phase 0 输出提示，不阻塞 Phase 1-4。进入 Phase 5 前向用户提问并提供安装选项（「是，帮我安装」/「跳过」/「查看详情」）
- **OpenRouter 未就绪**：提示执行 setup 工作流配置（Key 存储在环境变量），不阻塞
...
> 目标：自动检测产品中的非 CRUD 技术点，通过网络搜索调研现有方案，向用户呈现 2-3 个方案的对比表格，由用户选择。
...
各 spike 类别互不依赖，**并行执行以下 N 个任务**（N = 检测到的 spike 数量），每个任务负责一个 spike 类别的 2-3 轮网络搜索 + 方案提炼。
...
若 OpenRouter MCP 可用（`mcp__ai-gateway__ask_model` 工具存在），对每个 spike 向不同模型发送验证请求：
...
> 若 OpenRouter 不可用 → 跳过此步，仅使用网络搜索结果。
...
对每个 confirmed spike，LLM 判断需要什么凭据，向用户提问一次性收集：
...
- 可以稍后补充（执行 project-forge credentials 工作流更新）
```
...
加载 `skills/project-setup.md`，按其工作流执行。
...
加载 `skills/design-to-spec.md`，按其工作流执行（Step 1-6 规格生成 + Step 7 共享层分析）。

design-to-spec 内部并行加速：后端子项目先完成 spec，然后多个前端子项目并行生成 spec。详见 design-to-spec.md 的「并行执行编排」段落。
...
加载 `skills/task-execute.md`，按其工作流执行。
...
互不影响，**并行执行以下 4 个任务**：
  ...
  任务 1: 完整产品验收
    加载 skills/product-verify.md
    执行 product-verify full 工作流（静态 + 动态）
    输出: verify-tasks.json（IMPLEMENT / REMOVE_EXTRA / FIX_FAILING）

  任务 2: 跨端验证（测试锻造 E2E 链）
    加载 commands/testforge.md
    执行 testforge 工作流（Phase 4 Path B: E2E chain forge）
    输出: testforge-report.md（E2E chain 结果）

  任务 3: 死链猎杀
    加载 commands/deadhunt.md
    执行 deadhunt static 工作流（静态分析）
    若应用已运行 → 执行 deadhunt full 工作流（含深度测试）
    输出: validation-report-summary.md + fix-tasks.json

  任务 4: 字段一致性检查
    加载 commands/fieldcheck.md
    执行 fieldcheck full 工作流
...
加载 `commands/testforge.md`，执行 Phase 4 Path B（E2E chain forge）。
...
1. [ ] 处理 DEFER 任务
2. [ ] 修复 E2E 失败项
3. [ ] 执行 demo-forge 工作流灌入演示数据
4. [ ] 执行 code-tuner full 工作流进行架构分析
