...
Phase 3: 目标确认          展示源码全貌，锁定：                             ← 停顿
    ...
    │                      ⑤ 目标生态 7 层对齐（自动：通信/调度/数据流/错误/分层/配置/暴露）
    │                      ⑥ 分析粒度（自动决定）
    ↓ 用户确认 → 目标锁定
...
**`--from-phase N` 选项**：用户可指定从某个 Phase 重跑（如源码更新后只需重跑 Phase 4）。此时保留 `replicate-config.json` 中已有配置和之前 Phase 的产物，仅从指定 Phase 开始重新执行。`steps_completed` 中大于等于 N 的记录清除。示例：`/code-replicate --from-phase 4` → 保留 Phase 1-3 配置，从 Phase 4 重新分析。
...
> **核心原则**：复刻的输入是源系统的**业务意图**（做什么），不是源系统的**实现决策**（怎么做）。实现方式由目标生态的架构惯例填充。任何一层如果跳过，都可能导致实现与生态不兼容，且这类问题通常只在端到端联调时才暴露。

> **与 Phase 5b 的边界**：3d 决定每一层的**策略方向**（adopt_target / preserve_source / hybrid）；Phase 5b 决定具体技术选型（如 L2 决定"采用目标"后，5b 才选"asynq vs machinery"）。3d 回答"这一层跟谁走"，5b 回答"具体用什么"。

用户确认目标技术栈后，**自动执行**以下七层检查（不问用户）。每一层先在目标生态中找最相似的已有实现作为参考，提取惯例，再映射源系统的业务意图。产出写入 `replicate-config.json` 的 `ecosystem_alignment` 字段。
...
- 服务间调用的协议是什么？
- 请求的生命周期是多长？
- 响应是在同一请求上下文内返回，还是通过独立通道回传？
- 如果通过独立通道，请求与响应如何关联？
- 是否存在"对外同步、对内异步"的模式？如果是，边界在哪里？

#### 第二层：任务调度模式

检查问题：
- 有哪些任务类型（定时 / 事件驱动 / 持续消费）？
- 每种任务类型用什么框架注册和管理？
- 任务的执行入口是阻塞还是非阻塞？生命周期如何与上下文绑定？
- 并发控制在哪一层，最大并发如何配置？
- 任务失败后的行为是什么（重试 / 跳过 / 告警）？
- 分布式场景下如何避免多实例重复执行？
...
检查问题：
- 目标生态采用什么分层模式，各层职责边界是什么？
- 外部依赖在哪一层引入？
- 业务规则、参数校验、数据访问分别属于哪一层？
- 一致性边界在哪一层管理？
- 依赖的声明、组装、注入遵循什么约定？
...
- 配置缺失或为空时，应该降级还是报错？
- 本地开发与生产环境的差异如何处理？
- 端口、服务名、运行模式有哪些已有约定需要遵守？

#### 第七层：对外暴露约定

检查问题：
- 新功能是否需要在入口层注册？注册在哪里、如何注册？
- 服务注册与发现遵循什么约定？
- 是否有统一的认证 / 鉴权机制需要接入？

#### 执行方式

**步骤 1**：扫描 Phase 2 产出的 `source-analysis.json`，提取源系统在七层中每一层的实现方式。

**步骤 2**：根据目标技术栈，在目标生态中找最相似的已有实现（框架官方示例 / 社区主流实践），提取每一层的惯用方式。

**步骤 3**：逐层对比，生成 `ecosystem_constraints` 列表。每条约束：

```json
{
  "layer": "L1_communication",
  "source_approach": "PHP 同步阻塞，单次请求内返回",
  "target_convention": "Go goroutine，对外同步对内异步，边界在 Service 层",
  "alignment_action": "adopt_target",
  "rationale": "业务逻辑保留，通信层采用目标生态模型"
}
```
...
**对 Phase 4 的影响**：Phase 4 深度分析时，`alignment_action: adopt_target` 的层，仅从源码提取**业务逻辑**（做什么），不提取实现方式（怎么做）。`behavior-specs.json` 中对应行为的 `implementation_notes` 标注 `[TARGET_CONVENTION:{layer}]`，表示该层实现方式由目标生态决定。
...
| 层 | 源系统 | 目标生态 | 决策 |
|----|--------|---------|------|
| L1 通信模型 | 同步阻塞，单请求返回 | goroutine，对外同步对内异步 | ✅ 采用目标 |
| L2 任务调度 | Celery 事件驱动 | asynq + Redis | ✅ 采用目标 |
| L3 数据流转 | RabbitMQ，JSON，手动 ACK | Kafka，Protobuf，自动提交 | ⚠️ 待确认 |
| L4 错误处理 | 自定义错误码枚举 | 标准 error 接口 + sentinel | ✅ 采用目标 |
| L5 分层边界 | MVC + Repository | Clean Architecture + DI | ✅ 采用目标 |
| L6 配置环境 | .env + dotenv | Viper + YAML | ✅ 采用目标 |
| L7 对外暴露 | 手动路由注册 | 自动 wire + gRPC 注册 | ✅ 采用目标 |
```
...
输出「Phase 3 ✓ {N} 模块纳入 | 目标: {target_stack} | 方向: {replicate/slim/extend} | 7 层对齐: {N} adopt / {N} hybrid | 粒度: {fine/standard/coarse} | 产物: {N} 个文件 | 开始深度分析（不再停顿）」，自动继续 Phase 4。
...
>
> **与 Phase 3d 的边界**：3d 已确定每层的策略方向（adopt_target / preserve_source / hybrid），本步骤在 `adopt_target` 的层中选择具体技术方案。3d 回答"这一层跟谁走"，5b 回答"具体用什么"。
...
| **生成文件** | ORM migration、接口文档生成物、proto 编译产物、框架脚手架 | 后提交，review 时可跳过 |
