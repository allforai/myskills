...
LLM 审查 extraction-plan 的覆盖盲区，补充遗漏的 key_files。

> 详见 ${CLAUDE_PLUGIN_ROOT}/docs/phase3/extraction-plan-review.md
...
LLM 对每个 screen 自主决定 stub（快速）或 deep（完整）分析深度。deep 模式提取 interaction_triggers（5 种 trigger 类型）、state_variants（含远程事件驱动）、render_rules、global_components，并执行交互模式自查。

> 详见 ${CLAUDE_PLUGIN_ROOT}/docs/phase3/experience-map-deep.md
> Schema 详见 ${CLAUDE_PLUGIN_ROOT}/docs/schemas/experience-map-extensions.md
...
双维度兜底：源码覆盖（从文件出发）+ 用户体验走查（从用户出发）。

> 详见 ${CLAUDE_PLUGIN_ROOT}/docs/phase3/completeness-sweep.md
