...
> | 规则组文件 | 包含规则 | 适用 Agent |
> |-----------|---------|-----------|
> | `rules/base.md` | 1-4, 10, 12, 24, 28, **29**, **30** | 所有 |
> | `rules/convergence.md` | 5-9, 11, 13 | Forge agents |
> | `rules/e2e.md` | 14-23, 27, **31**, **32** | Path B/C/E |
> | `rules/data-linkage.md` | 25-26 | Path C + cr-visual |
>
> **注意**：Path A（单元/组件测试）已移除。LLM 代码的 bug 在接缝层不在函数内部。
...
### 11. 降级不阻断
无上游文档时降级执行（仅代码级审计 + 集成测试锻造），不阻断整个流程。
...
### 20. 禁止手写 Mock API 响应（被 #29 取代，保留编号兼容）

**已升级为铁律 #29。** 原规则"mock 数据必须反映真实格式"实践中无法执行 — LLM 按 TypeScript 类型定义生成 mock，类型定义本身就和后端不一致时 mock 也跟着错。

**新规则**：不 mock HTTP API 响应。用 API fixture 录制（curl 真实 API）或连真实后端。详见 #29。
...
**testforge 的优先级应该是：Chain 0 接缝验证 > E2E 操作链 > 业务链 E2E > 集成测试。**
不生成单元测试 — 先确保用户能打开页面、能登录、能操作、能看到数据，再去测边界条件和异常路径。
...
每个跨网络 Action 必须测 4 种动态边界状态，全部用真实手段触发：

| 状态 | 真实触发 | 验证 |
|------|---------|------|
| Pending | Playwright `network.throttle('Slow 3G')` | skeleton/spinner？按钮 disabled？ |
| Resolved | 正常操作 | 数据正确？乐观更新一致？ |
| Rejected | 畸形数据触发 4xx / 停依赖服务触发 5xx | 友好提示？表单数据保留？ |
| Timeout | `browser.offline` / 停后端 | 超时提示？离线恢复数据不丢？ |

**禁止 mock 响应结果** — mock 500 返回标准 JSON，前端处理通过；真实 500 可能返回 HTML → JSON.parse 崩溃 → bug 逃逸。真实触发才能暴露真实行为。

### 28. 反惰性断言 — 验证对错而非有无（Anti-Lazy Assertion）

LLM 天然倾向生成存在性断言（Level 0/1），但这类断言几乎不能发现 bug。

**断言深度**：Level 0 存在性 → Level 1 量化性 → Level 2 结构性 → **Level 3 值正确性**（目标）。

**强制比例**：
- 业务逻辑测试（integration）：Level 3 ≥ 60%
- E2E/platform_ui：Level 3 ≥ 40%，Level 2+ ≥ 80%
- 全 Level 0/1 的测试 → 拒绝重写

**核心原则**：`expect(有)` 不等于 `expect(对)`。一个显示了错误用户数据的页面也"有数据"，但数据是错的。

### 29. 禁止 Mock — 真实依赖或不测（No Mock, Real Dependencies or No Test）

所有测试必须连真实依赖。依赖不可用 = NOT_TESTED，不生成 mock 测试。

**禁止 mock**：项目内部可控的一切依赖 — 数据库、缓存、后端 API、service 层、repository 层、前端 store。
**唯一例外**：不可控的外部第三方（支付/OAuth/短信/外部 AI），限定 adapter 层 mock，标注 `EXTERNAL_MOCKED`。
**为什么**：mock 编造的数据和真实依赖行为不一致。接缝层 bug 只有真实联调能发现。没有真实依赖的测试不如不测。

### 30. E2E 操作链最高优先级（Operation-First, No Unit Tests）

E2E 操作链（Path E）排在 Chain 0 之后**立即执行**。不生成单元/组件测试（Path A 已移除）。

**为什么移除 Path A（单元/组件测试）**：
- 实战数据：400 个 unit 测试全绿 + 0 个真实 bug 发现；1 条 E2E 操作链 5 分钟 + 发现"删除有关联数据时前端报错"的接缝 bug
- LLM 生成的代码中，bug 集中在接缝层（前后端字段不一致、CORS、JWT、状态同步），不在单个函数内部
- 单元测试 mock 掉了接缝，所以恰恰测不到真正有 bug 的地方
- E2E 操作链测的是**用户真实操作路径**（点击按钮 → API 调用 → 错误处理 → toast 提示），这才是 bug 密度最高的层

**执行方式**：Path E 在 Chain 0 之后立即执行。Path E 发现的 BIZ_BUG 立即修复，不进入 CG-1 轮次计数。

### 31. CRUD 负向操作必测（Negative CRUD）

对每个 delete/update/状态变更操作，除了正向路径，**必须测至少一个负向路径**。

| 操作 | 正向（必测） | 负向（必测） |
|------|------------|------------|
| DELETE | 删除无关联实体 → 验证消失 | **删除有关联数据的实体 → 验证 409 拒绝 + 错误 toast + 数据仍在** |
| UPDATE | 修改为有效值 → 验证变化 | 修改为无效值 → 验证 400 + 表单错误提示 |
| STATE | 合法转换 → 验证 badge 变化 | 非法转换 → 验证拒绝 |
| CREATE | 有效数据 → 验证出现 | 重复/无效数据 → 验证 409/400 + 错误提示 |

**为什么负向 DELETE 是最高优先级**：
数据库有 FK 约束 → 后端返回 409 CONFLICT → 前端 error interceptor 解析 message → mutation onError 触发 toast → ConfirmDialog 状态管理。
这条链路有 5 个接缝节点，任何一个断裂用户就看不到错误提示。
实战证明：正向删除测试全绿，但删除有关联分类时前端 ConfirmDialog 的 `onConfirm` 异步函数吞掉了 `mutateAsync` 的异常 — 因为没有 try/catch。
**这种 bug 单元测试永远测不到**（不经过真实 HTTP → interceptor → mutation → UI 链路）。

### 32. E2E 链禁止只读（No Read-Only Chains）

如果一条 E2E 链里**没有任何写操作**（create/update/delete/approve/reject/reply），它不是 E2E 链，是烟雾测试。

```
只读链（禁止标记为 E2E 链）：
  ✗ 导航到列表页 → 验证有数据 → 导航到详情页 → 验证有内容
  ✗ 登录 → 访问 10 个页面 → 每页检查 heading 存在

操作链（合格的 E2E 链）：
  ✓ 创建分类 → 验证出现在树中 → 编辑名称 → 验证变化 → 删除 → 验证消失 → 审计日志有记录
  ✓ API 种子 draft 场景包 → UI 审批通过 → 验证状态变 Published → 归档 → 验证状态变 Archived
```

烟雾测试有价值但属于 smoke-all-pages.spec.ts，不属于 E2E 链。
E2E 链的核心价值 = **验证操作的因果关系跨页面正确传播**。
