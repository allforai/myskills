...
`$ROOT` = this skill directory。四镜头：`$ROOT/lenses.md`；
实测官 / 普查官 / 枚举官 prompt：`$ROOT/prompts/{prober,census,sites}.md`；schema：`$ROOT/schemas.md`；
...
6. 使用 `ledger_store.py` 加锁并初始化/载入 `ledger.json`（schema 见 `$ROOT/schemas.md`）。
...
1. **独立 surface 枚举**：派一个 fresh-context agent，prompt = `$ROOT/prompts/census.md` 全文 +
...
2. **合并 + 摆面**：把 census 面与你自己想到的面合并去重，**标出"census 有、你没想到"的面**（那
   往往正是盲区）。然后一次只问用户一个选择，确定盘哪些、先盘哪个。
...
- **派实测官**：`spawn_agent` with `fork_turns:"none"`, followed by `wait_agent`，prompt = `$ROOT/prompts/prober.md`
...
- **孤例还是一类（每条 `gap|drift` 落账时必答）**：这个缺口是这一处独有，还是某个结构模式的
  ...
  ② 派**同类位点枚举官**（fresh-context，只枚举不取证，成本一问一 agent）：prompt =
...
- **实测官死/超时**：重派一次（基础设施失败不算数）；再失败该问记 `could_not`
  证据（原因文件），裁 `unprovable`，绝不编造。**普查官/枚举官无返回**同样重派一次；普查官再失败
