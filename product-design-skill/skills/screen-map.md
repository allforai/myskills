...
- **交互类型推断**：按 `docs/interaction-types.md` 的推断规则，根据每个界面的 actions 自动标注 `interaction_type` 字段。组合类型用数组表示（如 `["MG5", "MG3"]`）。此字段供下游 ui-design 和 design-to-spec 使用

#### 界面 Schema

> 详见 ${CLAUDE_PLUGIN_ROOT}/docs/schemas/screen-map-schema.md（含界面字段说明、按钮字段说明）
...
> **搜索驱动原则**：展示界面分类选项前，先 WebSearch 搜索「{产品类型} UI information architecture best practices」，用搜索结果校验界面分类的合理性，确保不遗漏行业通用界面模式。
...
> screen-index.json schema 详见 ${CLAUDE_PLUGIN_ROOT}/docs/schemas/screen-map-schema.md
