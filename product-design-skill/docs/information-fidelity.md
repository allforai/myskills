...
> 目标：降低 `concept → map → screen → use-case → gap → ui-design → audit` 的信息丢失与失真。
...
- **工具检测**：每个阶段在指定 Step 完成后，检测 `mcp__plugin_product-design_ai-gateway__ask_model` 工具是否可用。
- **可用时**：自动发起交叉验证调用，将结果写入产出的 `cross_model_review` 字段。不停下来询问用户。
- **不可用或调用失败时**：自动跳过，不阻塞流程，不生成 `cross_model_review` 字段。产出与未启用 XV 时完全一致。

### 调用方式

使用 `mcp__plugin_product-design_ai-gateway__ask_model` 工具，传入：
...
- `experience-map`：每个界面附「用户目标 + 失败恢复 + 风险提示」。XV：Step 2 后由 gpt 审查 UX 一致性、gemini 检查无障碍缺陷。
...
| 失真模式 | 发生阶段 | 防护措施 |
|---------|---------|---------|
| 创新稀释 | experience-map / ui-design | 标注 `innovation_screen` + `adversarial_concept_ref` |
...
| 层级 | 阶段 | 检测内容 | 修复策略 |
|------|------|---------|---------|
| **L1 预防** | product-map | 状态供需平衡（孤儿/幽灵状态） | 上游补充缺失的生产者/消费者任务 |
| **L2 检测** | feature-gap | 状态流转路径追踪（断裂/死循环/分支缺失） | 中游补充缺失的转换节点或分支 |
| **L3 验证** | use-case | E2E 用例覆盖状态流转 | 下游补充测试用例覆盖未验证路径 |

**检测机制**：
- **product-concept**：定义创新概念的状态机（初始状态 → 中间状态 → 终止状态）
- **product-map**：粗粒度统计（0 次引用/0 次产生的状态）+ 状态供需平衡检查
- **feature-gap**：细粒度验证（孤儿状态/幽灵状态/语义鸿沟/创新概念状态缺口）+ 状态流转路径追踪
- **use-case**：动态验证（E2E 用例覆盖状态流转）

**修复优先级**（通用规则）：
1. **幽灵状态**（有消费无生产）：**P0 级修复**（零容忍，状态永远无法触发）
2. **孤儿状态**（有生产无消费）：**P1 级修复**（数据孤岛，可能导致数据泄漏）
3. **状态流转断裂**：**P1 级修复**（用户操作卡住，无法继续）
4. **分支缺失**：**P2 级修复**（如"审核拒绝"无处理，用户不知道结果）
5. **语义鸿沟**：**P3 级优化**（建议统一命名或建立映射）
