...
> | 规则组文件 | 包含规则 | 适用 Agent |
> |-----------|---------|-----------|
> | `rules/base.md` | 1-4, 10, 12, 24, 28, **29** | 所有 |
> | `rules/convergence.md` | 5-9, 11, 13 | Forge agents |
> | `rules/e2e.md` | 14-23, 27 | Path B/C |
...
### 29. 禁止 Mock API 响应（No Mock API Responses）

前端 unit/component 测试禁止 mock HTTP API 响应。Mock 数据按前端类型定义编造，真实 API 返回的字段名、类型、是否可空全部可能不一致。Mock 测试只能证明"前端代码在理想数据下不崩溃"，证明不了"前端和后端能协同工作"。

**允许 mock**：后端 service 层 mock repository 接口（内部实现细节，不涉及接缝）。
**禁止 mock**：HTTP API 响应、JWT 结构、前端 store 初始状态注入、HTTP 拦截器（msw/nock/axios-mock-adapter）。
**没有真实后端 = 标记 NOT_TESTED**，不生成假绿的 mock 测试。

**为什么**：mock 数据按前端类型定义编造，真实 API 的字段名/类型/可空性全部可能不一致。数百个 mock 测试全绿但 E2E 连真实后端一跑多页崩溃是反复出现的模式。接缝层 bug 只有真实联调能发现。
