...
Step 1: 频次过滤（帕累托）
      ↓ 用户确认频次评估
Step 2: 场景对齐
      基于 .allforai/screen-map/screen-map.json（若不存在则自动运行 screen-map）
      ↓ 用户确认场景归属
Step 3: 竞品参考（可选）
      ↓ 用户确认参考结论
...
**场景匹配方法**：读取 `screen-map.json` 中每个界面的 `primary_action` 和 `pareto.high_freq_actions`。任务的主操作出现在高频操作列表中 → 核心场景；出现在界面但非高频 → 次要场景；未出现在任何界面 → 无对应场景。若 `screen-map.json` 不存在，前置检查已自动触发 screen-map 运行，此处 screen-map 必已存在。
