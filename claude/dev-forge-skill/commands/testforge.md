---
description: "测试驱动的质量锻造：审计测试缺口 → 补测试 → 修 bug → 直到全绿。覆盖测试金字塔（integration / platform-ui / e2e-operation / e2e-chain）。模式: full / analyze / fix"
...
```
    ...
    ╱ E2E Operations╲      ← 单站 CRUD 操作链 + 正向/负向（Path E ⚡最高优先级）
   ╱  Platform UI    ╲     ← 同一场景 × N 个平台的 UI 自动化（Path C）
  ╱   Integration     ╲    ← 多模块协作测试（Path D）
```

> **不生成单元/组件测试（Path A 已移除）**：LLM 生成的代码中，bug 集中在接缝层和集成层，
> 不在单个函数内部。实战数据：400 个 unit 测试全绿 + 0 个真实 bug 发现。
> 执行顺序：Chain 0 → **Path E（操作链，最高 ROI）** → Path D/C/B。
...
**Layer E: 接缝+动态负空间**（始终执行）— 组件交界处的运行时行为

> Layer A-D 关注的是"功能有没有做对"，Layer E 关注的是"接缝处的动态行为有没有守护"。

**E1: 中断恢复**（对每个 CRUD 操作推导）：
- 操作中途网络断开 → 恢复后表单数据是否保留？
- API 返回 500 → 用户点重试 → 不会重复提交（幂等性）？
- 页面刷新 → 未提交的草稿是否可恢复（localStorage/sessionStorage/draft API）？
- 长表单填写中途 session 过期 → 重新登录后数据是否丢失？

**E2: 并发操作**（有 role-profiles 且同一实体多角色可写时推导）：
- 同一实体的并发编辑 → 后保存是否覆盖先保存？有乐观锁/版本号吗？
- 审批中的状态竞争 → A 审批通过 vs B 同时驳回 → 最终状态是否确定性？
- 库存/余额/名额的并发扣减 → 是否有超卖/超扣风险？
- 删除 vs 编辑竞争 → A 正在编辑的记录被 B 删除 → A 提交时会怎样？

**E3: 依赖降级**（有 infrastructure-profile 时执行）：
- 对 infrastructure-profile 中每个外部依赖（数据库、缓存、搜索引擎、消息队列、第三方 API）：
  - 该依赖不可用时 → 前端显示什么？应该是友好错误提示，不是白屏/500/堆栈追踪
  - 该依赖响应超慢（>10s）→ 有超时处理吗？用户看到 loading 还是挂起？
  - 该依赖返回错误格式 → 前端能优雅降级还是 JS 报错？
...
→ 输出进度：「Phase 3 负空间 ✓ Layer A:{a} B:{b} C:{c} D:{d} E(中断:{e1} 并发:{e2} 降级:{e3})」
...
**追加项锻造**：汇总 OL-D1/D2/D3 的追加缺口，遵循 D → C → B 顺序再跑一轮 Phase 4。
...
| 26 | 控件联动必须显式测试因果：操作 A → 验证 B 正确响应，禁止只测最终状态 |
| 27 | 异步接缝四态测试：每个跨网络 Action 测 Pending/Resolved/Rejected/Timeout，零 mock 真实触发 |
| 28 | 反惰性断言：验证对错而非有无，业务测试 Level 3 断言 ≥60%，全 Level 0/1 直接拒绝重写 |
| **30** | **E2E 操作链最高优先级：Path E 排在 Chain 0 之后立即执行** |
| **31** | **CRUD 负向操作必测：每个 delete/update/状态变更必须测正向+负向，删除有关联数据是最高优先级** |
| **32** | **E2E 链禁止只读：没有写操作的链是烟雾测试不是 E2E 链** |
