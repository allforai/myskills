...
| 2 | 未回答的问题保留待决状态；中断或旧文件存在不产生隐式确认。 | `test_product_intent_session.py::test_resume_raises_legacy_inference_as_missing_confirmation` (the plan cited the wrong file; corrected here); `test_legacy_profile_authority.py::test_recording_legacy_confirmation_cannot_invent_a_new_consent_source` | PASS (2 passed in 0.36s — host-parametrized, rerun after correcting the citation) |
...
| 9 | 对齐 main 的一次性 Run Policy：两端首节点前采集并持久化运行策略，恢复复用；重试/迭代/安全告警消费已记录策略，不在运行中重新访谈。 | `claude/meta-skill/tests/unit/test_run_policy_session.py::test_policy_is_collected_once_reused_and_never_confirms_product_requirements`; `::test_generated_driver_consumes_iteration_policy_without_new_interviews` | PASS (2 passed in 0.80s — first cited test; all cited tests run and passed) |
| 10 | 运行策略不是产品决定；continue、accept 或已有策略文件都不能批准待决需求。缺少必要产品决定时阻止受影响工作并返回交互入口。 | `claude/meta-skill/tests/unit/test_run_policy_session.py::test_policy_is_collected_once_reused_and_never_confirms_product_requirements`; `test_bootstrap_scope.py::test_unanswered_local_requirement_blocks_both_run_preflight_gates` | PASS (2 passed in 0.80s — first cited test; all cited tests run and passed) |
| 11 | 本票负责完整确认状态的续接/反悔/旧基准迁移；#9 的有效局部 journal 消费缺陷必须在 #9 关闭，不能推迟到本票。 | scope statement — proved by #9 record rows 9–10 (journal projection at all gates), not by this ticket | PASS (by reference to #9 record rows 9–10) |
