...
description: >
  ...
  Scans task-inventory and experience-map to identify recurring functional patterns (CRUD台、列表+详情、审批流、
...
> 扫描 task-inventory + experience-map → 识别重复设计模式 → 用户确认 → 输出 pattern-catalog.json（含标签映射）+ pattern-report.md

## 目标

在并行设计开始前识别产品中的重复功能模式，通过统一模板避免"同模式多套路"：

1. **扫描** — 读取 task-inventory.json + experience-map.json，检测 8 类功能模式
...
Phase 4.5: design-pattern → pattern-catalog.json ← 本技能
Phase 5-8: 并行组（ui-design 消费 pattern-catalog） ← 下游
Phase 9: design-audit → Pattern Consistency 维度 ← 事后
```
...
Preflight: 检查前置文件存在性
    ↓ 自动
Step 1: 模式扫描（扫描 task-inventory + experience-map，检测 8 类模式）
    ↓ 自动
...
> 目标：检测 task-inventory + experience-map 中的 8 类功能模式。

**扫描维度**：task 标题、task type、task actions（experience-map 中的 actions 字段）、business-flows 节点序列、roles（涉及多角色的实体）。
...
| `PT-LIST-DETAIL` | 列表+详情对 | experience-map 中同模块存在 list 类型界面 + detail 类型界面 | 2+ 对 |
| `PT-APPROVAL` | 审批流 | business-flows 中出现 submit→review→approve/reject 节点序列 | 1+ 条流 |
| `PT-SEARCH` | 搜索+筛选+分页 | experience-map actions 同时含 search/filter/query + paginate/page | 2+ 界面 |
...
用户确认后，自动执行以下操作，不再停顿。不修改任何上游文件（task-inventory.json、experience-map.json）。
...
不修改上游文件（task-inventory.json、experience-map.json）。下游技能通过 pattern-catalog.json 中的 `task_tags` / `screen_tags` 查找模式标签。
...
pattern 数据写入独立文件 pattern-catalog.json（含 `task_tags` / `screen_tags` 映射），不修改 task-inventory.json 和 experience-map.json。
