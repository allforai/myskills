...
同步检测 API 缺口：有实体无对应创建端点、有任务无对应 API，标记为 `API_GAP` → `API_MISSING_BLOCKER`。API 缺失的实体必须先补 API 再灌数据，不可绕过直写数据库。缺口实体输出到 `.allforai/demo-forge/api-gap-tasks.md` 供 dev-forge 执行补建。
...
若实体在 `api-gaps.json` 中标记为 `API_GAP`，标记为 `API_MISSING_BLOCKER` — 必须先补 API 再灌数据，不可绕过。缺口实体输出到 `.allforai/demo-forge/api-gap-tasks.md` 供 dev-forge 补建。
