...
"view_modes": [
  {
    ...
    "label": "全量列表",
    "trigger": "初始加载 / 清除筛选",
    "layout": "全部Tab高亮，表格显示所有订单，右侧无面板",
    "active_components": ["order-tabs", "search-bar", "order-table", "pagination"],
    "available_actions": ["搜索", "切换Tab", "点击行查看详情"]
  },
  {
    "id": "filtered_list",
    "label": "按状态筛选列表",
    "trigger": "点击状态Tab（待处理/已发货/已完成/退款中）",
    "layout": "选中Tab高亮，表格仅显示该状态订单，批量操作按钮出现",
    "active_components": ["order-tabs", "search-bar", "filtered-table", "batch-action-bar"],
    "available_actions": ["批量发货", "批量打印", "导出", "搜索", "点击行"]
  },
  {
    "id": "detail_panel",
    "label": "订单详情侧边栏",
    "trigger": "点击列表中某一行",
    "layout": "列表缩窄至左侧60%，右侧40%滑出订单详情面板",
    "active_components": ["order-table-narrow", "order-detail-panel", "action-buttons"],
    "available_actions": ["发货", "取消订单", "查看物流", "关闭面板返回全宽列表"]
  }
]
```
...
11. **填充 `view_modes`**：对 `operation_profile.screen_granularity = "multi_function_per_page"` 角色的屏幕，设计屏幕内的视图模式流转。描述用户操作如何触发视图变化（全量列表 → 筛选列表 → 详情面板 → 编辑模式等），每个模式说明布局变化和可见组件
...
6. **治理风格驱动设计**：读取 `governance_styles`，对每条业务流按其治理策略设计：
   - **严格管控**的流程 → 必须有审核队列、状态跟踪、审核员工作台
   - **宽松高效**的流程 → 不需要审核屏幕，改用举报/申诉通道、违规记录
   - **system_boundary.external** 中的功能 → 不在本系统屏幕中出现为可编辑表单（如"实名认证"是外部系统，注册表单只需基本信息）
...
| **功能-上下文适配** | 屏幕中的功能组件必须在该屏幕的业务上下文中合理 | LLM 逐屏审查：每个 component 和 action 是否在该屏幕的业务场景下有意义。例：购物车不应有"热门/最新"筛选（那是商品列表的功能）；注册页不应有完整的实名认证表单（如果 `governance_styles` 指定该流程为"宽松高效"） |
| **概念治理规则回溯** | 屏幕设计必须符合 `product-concept.json` 中对应业务流的 `governance_styles` | 对每个涉及审核/审批的屏幕，检查对应业务流的治理风格：(a) 治理风格为"宽松高效"→ 不应有审核队列屏幕 (b) 治理风格为"严格管控"→ 必须有审核+状态跟踪屏幕 (c) `system_boundary.external` 中的功能不应在本系统屏幕中出现为可编辑表单 |
...
| **跨应用数据泄露** | 屏幕不应展示只属于其他应用的数据 | 检查规则：(a) app=merchant 的屏幕不应包含"全平台统计"、"所有商户"等平台级数据字段 (b) app=admin 的屏幕不应包含单个商户的日常运营数据（如"我的订单"） (c) app=website 的屏幕不应包含后台管理字段（如"审核状态管理"） |
...
结构正确（字段存在、状态数量够）但业务错误（购物车加了商品筛选、注册页加了不该有的认证表单）的屏幕比结构不完整更危险——因为后者容易被自动检测，前者只能被人工审核发现。LLM 设计每个屏幕时必须回答：「这个功能/组件/字段在这个业务场景下是否合理？」概念阶段收集的治理风格和操作频度是判断合理性的基准。
