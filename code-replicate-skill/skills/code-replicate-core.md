...
更新 `replicate-config.json`：scope_filter, target_stack, business_direction, analysis_granularity, expected_outputs。

输出「Phase 3 ✓ {N} 模块纳入 | 目标: {target_stack} | 方向: {replicate/slim/extend} | 粒度: {fine/standard/coarse} | 产物: {N} 个文件 | 开始深度分析（不再停顿）」，自动继续 Phase 4。
...
将 5a/5b/5c 中需要用户输入的问题，合并成**最多 4 个** `AskUserQuestion` 调用（选项化，不开放填写），收集用户决策。
