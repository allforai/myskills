...
# ── Interaction type inference (retired) ──────────────────────────────────────
# The MG/CT catalog is gone. Do not stamp type codes. Callers may still unpack
# this tuple; the first element is always empty.


def infer_interaction_type(task, crud_type="", audience_type=""):
    """Retired. Always returns ("", crud_hint). Do not assign MG/CT codes."""
    _ = (task, audience_type)
    hint = crud_type if crud_type in {"C", "R", "U", "D"} else ""
    return "", hint


def refine_view_type(task, crud_type):
    """View-type hint from CRUD only. Does not use the retired type catalog."""
    _ = task
    return {
        "C": ["create_form"],
        "U": ["edit_form"],
