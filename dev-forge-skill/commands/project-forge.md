...
> 静态分析无法发现：运行时导入失败、跨服务配置不匹配、页面渲染错误、CORS 拦截等问题。
> 这些问题必须通过**启动应用 → 实际请求 → 检查结果**来发现。
>
> **不硬编码检查项**——让 LLM 读当前项目的代码和配置，推理出该项目可能存在哪些运行时风险，
> 然后生成对应的冒烟测试。不同项目的风险不同，生成的测试也不同。

前置：启动所有子项目应用（backend + 前端 dev server）。
...
- 每个子项目的入口文件（main.py / nuxt.config.ts / main.dart）
- 每个子项目的配置文件（.env / .env.example / config.py）
- 每个前端子项目的 API 调用模块（useApi.ts / api_client.dart / api_endpoints.dart）
- build-log.json 中的 files_modified 列表

然后回答：
1. 这些子项目之间在运行时如何连接？连接点可能断在哪里？
2. 前端调用的 API 地址/端口/路径在运行时是否与后端一致？
3. 代码中引用的第三方包导出（组件/函数/图标）在运行时是否存在？
4. 认证令牌在跨服务传递时链路是否完整？
5. 还有哪些该项目特有的运行时风险？
...
| 层级 | 验证什么 | 不通过说明什么 |
|------|---------|-------------|
| **进程级** | 每个子项目启动后能响应请求（后端 200、前端页面能打开且无致命 JS 错误） | 编译通过≠运行时正常（import 错误、配置缺失、依赖冲突）|
| **连接级** | 前端→后端的请求能到达且返回预期格式（不被 CORS/auth/路由拦截） | 子项目间的配置不一致（端口/路径/认证）|
| **数据级** | 至少一条核心业务的完整链路（创建→读取→列表出现） | API 实现与数据库模型不匹配（类型错误、字段缺失）|

在三个层级之上，LLM 根据项目特点自由补充更多测试——不同项目的风险不同，具体测试内容由 LLM 推理决定。
...
- NOT_TESTED: 无法执行（如 Playwright 不可用、应用未启动）
...
**违反此铁律的典型场景**：
- Phase 5 静态扫描通过 → 标记 completed → Phase 7 E2E 跳过 → 运行时 bug 逃逸。**错误**：静态通过应标记 `static_pass`，Phase 7 必须执行
- Playwright 不可用 → 标记 skipped → 报告显示"全部通过"。**错误**：应标记 `not_tested`，报告醒目提示未测试项数量
