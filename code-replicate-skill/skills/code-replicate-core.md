...
```json
{
  "vectors": [
    {
      "component": "LLM 自命名",
      "source_file": "源码中该组件的实现文件",
      "cases": [
        {"input": "...", "expected_output": "...", "description": "LLM 从源码测试或常量中提取"}
      ]
    }
  ]
}
...
**示例场景**（LLM 根据实际项目生成，以下仅为说明格式）：
- 加密组件：`input=明文 + key → expected_output=密文`
- 协议解析组件：`input=原始帧字节 → expected_output=解析后的结构`
- 路径构建组件：`input=原始路径 → expected_output=转换后的路径`
