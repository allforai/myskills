...
export async function resolveLatestModel(familyId: string): Promise<string | null> {
  ...
  if (candidates.length > 5) console.error(`    ... 还有 ${candidates.length - 5} 个`);

  const latestModel = await resolveLatestModelWithLLM(familyId, candidates);
  if (latestModel) await writeLLMCache(familyId, latestModel);
  return latestModel;
}

export async function resolveAllFamilies(): Promise<Record<string, { model_id: string | null; display_name: string }>> {
