...
| 阶段 | 名称 | 说明 |
|------|------|------|
| A | Static Analysis | 8 维度代码对比（F1-F8），输出静态评分 + 差距清单 |
| A2 | Runtime Verify | 构建 → 冒烟启动 → 测试向量 → 协议兼容性（R1-R4） |
| B | Fix | 按差距清单修复（运行时问题优先于静态问题） |
| C | Re-verify | 重新评分，不达标则回到 B，达标则输出终报 |

`full` 模式 = A → A2 → B → C 闭环（最多 3 轮）
`analyze` 模式 = 仅 A + A2
`fix` 模式 = 仅 B（基于上次 A/A2 的差距清单）
...
```
静态分 = (F1 + F2 + F3 + F4 + F5 + F6 + F7 + F8) / 有效维度数
```
...
执行目标项目的构建命令：
- `dotnet build` / `go build ./...` / `npm run build` / `flutter build` 等
...
尝试启动目标应用：
- 后端：启动 HTTP server → 检查 health endpoint 返回 200
- 前端：启动 dev server → 检查首页可访问
- 启动失败 → 标记 gap，附错误日志
- 无法独立启动（需要外部服务）→ 标记 `SMOKE_SKIPPED`，不扣分
- 评分：通过=100，失败=0，跳过=N/A

**R3 — 测试向量验证**（如果 test-vectors.json 存在）

跑 test-vectors.json 中的输入输出对：
- 对每个 vector：调用目标代码的对应函数 → 比对输出
- LLM 生成一个临时测试脚本执行 vectors
- 通过 → 该组件行为与源码一致
- 失败 → 标记精确的不一致点（哪个 vector 的哪个字段不匹配）
- 评分 = 通过的 vector 数 / 总 vector 数 × 100

**R4 — 协议兼容性测试**（如果有 `compatibility: exact` 的通信组件且服务端可达）

对 stack-mapping 中 `compatibility: exact` 的通信组件：
- 生成一个最小客户端测试脚本
- 向源码配置的服务端发送真实请求
- 验证：连接成功、认证通过、数据格式正确
...
```
静态分 = (F1 + F2 + F3 + F4 + F5 + F6 + F7 + F8) / 有效维度数
运行时分 = (R1 + R2 + R3 + R4) / 有效运行时维度数
综合分 = 静态分 × 0.5 + 运行时分 × 0.5
```

- R1 始终有效
- R2 无法启动时 N/A
- R3 无 test-vectors.json 时 N/A
- R4 服务端不可达时 N/A
- **综合分 < threshold 时，优先修运行时问题**（R1/R2 失败意味着代码根本不能用）
