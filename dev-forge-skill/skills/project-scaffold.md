...
前置: 加载
  ...
  ↓
  Forge 编排检测:
    .allforai/project-forge/forge-decisions.json 存在
...
Step 1: 脚手架规划（逐子项目）
  ...
  existing 模式:
    ...
    与模板定义 diff → 仅补缺文件列表
    绝不覆盖已有文件
  → Forge 模式: 输出进度「Step 1 脚手架规划 ✓ {N} 文件 ({breakdown by sub-project})」（不停）
  → 独立模式: 展示将创建的文件清单 → AskUserQuestion 用户确认
  ↓
...
Step 6: 启动验证
  Forge 编排模式（自动验证）:
    ...
    失败 → AskUserQuestion: 手动排查后重试 / 跳过验证
  独立模式（手动验证）:
    提示用户:
...
Step 6 启动验证不可跳过。Forge 编排模式下自动执行验证（pnpm install + mock 启动 + health check），仅失败时交互。独立模式下提示用户手动验证。
