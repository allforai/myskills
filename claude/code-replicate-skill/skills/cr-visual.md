...
视觉还原追求 **100% 一致**，不是"差不多就行"。持续修复直到完美。

> **外部插件增强**：如果 `ralph-loop` 插件已安装，可用 `/ralph-loop` 启动自动修复循环（无限轮次、自动诊断+修复+验证）。未安装时按以下内置流程执行。
...
**30 轮不是上限而是最低保证**：
- 60 个页面 x 可能每个页面需要 1-3 轮 → 需要足够多的轮次
- 如果 30 轮后还有 screen 未达 high → 继续（用户可选择继续或终止）
