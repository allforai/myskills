...
/feature-prune quick     # 快速剪枝，只看频次，跳过竞品
...
直接读取 `product-map` 中每个任务和按钮的 `frequency` 数据（无需重新收集）。
...
- `.allforai/feature-prune/prune-decisions.json` — 用户决策日志（增量运行复用）
...
只讨论该不该保留，绝不建议应该增加什么。发现功能缺口请使用 `feature-audit`（功能查漏）。
