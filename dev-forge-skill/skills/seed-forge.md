...
除经典理论外，执行种子数据锻造时通过 WebSearch 补充最新数据生成实践：

**搜索关键词模板**：
- `"{ORM} seed data strategies {year}"`
- `"realistic test data generation {language}"`
- `"faker.js alternatives {year}"`
- `"database seeding best practices {framework}"`

**来源优先级**：P1 官方文档 > P2 知名库文档（Faker, Factory Boy）> P3 技术博客 > P4 社区帖

**采纳决策**：记录到 `.allforai/seed-forge/trend-sources.json`，标注 ADOPT / REJECT / DEFER + 理由。
...
| 理论/框架 | 对应步骤 | 落地方式 |
|-----------|---------|---------|
| **Boundary Value Analysis** (Myers, 1979) | Step 1 数据设计 | 生成边界数据（空值、极值、特殊字符、极长/极短文本） |
| **Equivalence Partitioning** (Myers, 1979) | Step 1 数据设计 | 按角色/状态划分等价类，每类生成代表性数据 |
| **Property-Based Testing** (QuickCheck, 2000) | Step 2 数据变化 | 数据不千篇一律，属性在合理范围内随机变化 |
| **Unix Philosophy** — 文本流 | Step 3 数据格式 | 种子数据以 JSON 文本流转，上游输出 = 下游输入 |
...
| 业务约束 | 数据规则 |
|----------|----------|
| 子实体金额不超过父实体金额 | child.amount ≤ parent.amount |
| 高金额操作需要上级审批 | amount > threshold → approval.level = 2 |
| 某实体不可逆，无删除状态 | 不生成 status=deleted 的记录 |
| 审批有 SLA | created_at 和 approved_at 间隔在 SLA 范围内 |
