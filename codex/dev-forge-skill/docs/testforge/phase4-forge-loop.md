...
- render helpers 缺失 → 补
- E2E 配置缺失 → 根据探测到的 E2E 框架生成配置（Playwright 等）
- **前端测试必须连真实后端**（铁律 #29）。后端不可用 = 前端测试标记 NOT_TESTED，不生成 mock 测试
- 原则：复用已有风格，不另起炉灶
