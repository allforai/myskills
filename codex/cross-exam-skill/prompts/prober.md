...
2. **运行时取证逐状态截图**：`states_to_capture` 每个状态一张，存 `evidence_dir`，
   ...
   **截图有预算**：模型看图的有效分辨率是有限的，长边超过它的图会被缩回去，多出来的像素只是上传和 token；
   超过 API 单图上限的图更会让整轮调用作废、token 照扣。所以所有截图设备像素比设 1（高分屏的视口图也是），缺省拍视口一屏，整页只在状态要求整页时拍，
   设不了像素比就滚一屏拍一张（文件名带段号）而不是拍一张超长整页图——"每状态一张"是下限，一个状态多段不违规；截图工具会把图片内联进工具结果的（Playwright / DevTools MCP 缺省如此），先传 `filename` 落盘。落盘后的原图
