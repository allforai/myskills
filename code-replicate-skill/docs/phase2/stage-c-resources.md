...
**CRUD 全状态覆盖**（系统化，不手工挑选）：
LLM 读 source-summary 的每个模块 → 识别该模块管理的业务实体 → 为每个实体推导完整的 CRUD 交互截图：

- **Create**：打开创建表单 → 截图（空表单 + 必填标记）→ 填入测试数据 → 提交 → 截图（成功反馈）
- **Read**：列表页截图（有数据）+ 详情页截图 + 空状态截图（无数据时）
- **Update**：打开编辑表单 → 截图（预填数据 + 可编辑/只读字段区分）→ 修改 → 提交 → 截图
- **Delete**：点删除 → 截图（确认弹窗）→ 确认 → 截图（删除后列表更新）

**删除安全**：
- 优先使用 Create 步骤刚创建的测试数据来测 Delete（自产自销，不破坏已有数据）
- 如果系统有软删除 → 用软删除
- 如果无法安全删除 → 只截确认弹窗的截图，不执行实际删除，标记 `DELETE_SKIPPED_SAFETY`
