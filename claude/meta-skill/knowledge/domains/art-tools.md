...
def generate_placeholder_frame(width: int, height: int, label: str, output_path: str,
    ...
    """生成带尺寸标注的占位帧，用于 Spine/3D 资产的 human_gate 期间"""
...
art-gen 节点对不可自动化的资产（Spine/3D高模）生成规格文档：
...
```json
{
  ...
  "production_notes": "需 Spine 制作，参考关键帧参考图",
  "keyframes": [
    {"frame": 1, "desc": "地砖正常大小，完全不透明"},
    {"frame": 4, "desc": "缩放 1.3x，压扁变形"},
    {"frame": 8, "desc": "缩放 0.5x，收缩"},
    {"frame": 12, "desc": "透明度 0，粒子扩散"}
  ]
}
```
...
| 资产类型 | 原因 | 节点策略 |
|---|---|---|
| Spine 骨骼动画工程 | 骨骼绑定/动画调节需 Spine GUI | 生成帧序列规格 JSON + 关键帧参考图 |
...
| 角色立绘（卡通） | ✅ flux | - | - | - | QA |
| 角色骨骼分层参考 | ✅ flux(分层参考) | - | - | - | QA → Spine制作 |
| Spine 骨骼动画 | ❌ | ❌ | ❌ | ✅ 规格JSON | ✅ Spine制作 |
