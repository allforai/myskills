# megastorm — headless agent JSON schemas (Grok port)

Each Grok headless agent emits descriptor-gated NDJSON whose assistant chunks
assemble to exactly one JSON object matching its role schema.
...
```json
{ "type": "object", "required": ["kind", "verdict", "summary", "rerun_exit_code", "evidence"],
  "properties": {
...
```
Rule: `verdict:"confirmed"` is allowed ONLY if the supervisor independently reran `acceptance_cmd`
...
matched nothing); it forces `verdict:"rejected"` and signals §1.6 to re-inject the
...
defect remains ordinary `verdict:"rejected"`.
