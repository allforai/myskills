...
- 每个 `DEFER/CUT` 决策建议补充：`source_refs`、`constraints`、`decision_rationale`。
- 剪枝时建议至少覆盖 4/6 视角（`user/business/tech/ux/data/risk`），避免单一成本视角误剪。
- 对 `CUT` 项建议记录替代路径与延迟成本，降低下游理解偏差。

## 跨模型交叉验证（可选增强）

当 `mcp__openrouter__ask_model` 工具可用时，在 Step 4（用户分类决策）完成后自动发起交叉验证，结果直接写入产出的 `cross_model_review` 字段。
...
**自动写入内容**：误剪警告（被 CUT 的功能在竞品中为核心功能、被 DEFER 的功能延迟成本过高）、竞品参照（同类产品同阶段的功能保留率、行业趋势是否支持剪枝决策）。
