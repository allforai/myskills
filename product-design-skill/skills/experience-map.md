...
9. **审查并可调整** `data_fields`：骨架从 entity-model 机械复制字段，LLM 必须以用户视角逐字段审查两个问题——**该不该出现**和**能不能编辑**：
   ...
   - **能不能编辑**：同一个字段在不同界面类型下可编辑性不同。骨架的 `input_widget` 是从字段元数据机械推断的，不区分界面语境。LLM 须根据业务语义判断：创建时可填但创建后不可改的字段（如记录类型、用户名）在编辑表单中应为 `readonly`；由系统计算/状态机驱动的字段（如状态、评分、进度）在任何表单中都不应让用户手动输入
...
| **功能-上下文适配** | 屏幕中的功能组件必须在该屏幕的业务上下文中合理 | LLM 逐屏审查：每个 component 和 action 是否在该屏幕的业务场景下有意义。例：收藏夹不应有"热门/最新"筛选（那是内容列表的功能）；注册页不应有完整的实名认证表单（如果 `governance_styles` 指定该流程为"宽松高效"） |
