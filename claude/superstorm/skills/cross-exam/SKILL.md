...
**派子 agent 时默认不传 `model`，让它继承会话模型。** 实测官、普查官、枚举官、视觉 reviewer、复核官都是。
...
**降档只是可选的成本控制，且只许降取证类角色；模型名由定靶时读到的列表解析、由用户拍板，落在 ledger 顶层
`model_policy`，不落在技能里。** 技能给的是推荐规则（见定靶 0b：什么情况推荐降、降到哪一档），不是模型名。
用户选了降档时，只有"产出原始观察、盘问官自己会
解读"的角色（实测官、枚举官）可以用更便宜的模型；产出盘问官无法复核的判断的角色（普查官、视觉
reviewer）和作者自审时的复核官永远继承会话模型。哪怕降了档，也要遵守：
...
0b. **模型策略（一问，带推荐）**：先读本 harness 能派的模型列表（Claude Code：Agent 工具 `model` 的枚举；
   Codex：`codex --help` / 模型列表），按你对这些模型的了解从强到弱排好，找出**会话模型的下一档**——比会话弱一档、
   仍能独立跑服务和走浏览器的那个。然后摆两个都写着具体字面量的选项，并**明说你推荐哪个、为什么**：
   - A「证据优先」：全部继承会话模型。
   - B「成本优先」：取证类角色（实测官、枚举官、扫全实测官）用 <下一档字面量>，普查官、视觉 reviewer、复核官仍用
     会话模型。
   推荐规则：用户提过成本、或本 run 要走扫全模式（并行扇出十几个实测官，成本由取证主导）→ 推荐 B；旅程多、靶子要
   走弹窗 / 多标签 / OAuth 这类难走的界面 → 推荐 A，并说明降档实测官在这类旅程上更容易残缺重派（重派回会话模型，
   省的钱会赔回去）；两条规则相撞时推荐 A 并说明理由，用户仍可选 B。只推荐下一档，不推荐最便宜的：取证是整条链里最难的 agentic 活。字面量来自定靶时读到的列表，
   不来自记忆；强弱排序是你的了解不是价目表，在问题里说明这一点，用户可以推翻。用户选了 B 就写 ledger 顶层 `model_policy`（`observation` 字面量、`judgment` 固定 `"session"`、
   `confirmed_by_user` 用户原话、`confirmed_at`、`recommended` 你推荐的选项与一句理由）；选 A 也写，`observation`
   记 `"session"`。报告总览会点明本 run 取证用了哪个模型、谁在什么时候确认的、当时推荐的是什么；续盘沿用旧 ledger
   的策略，用户改口时把旧值推进 `model_policy.history[]` 再改键并更新原话与时间——前半段证据是谁取的不能从总览消失。
...
1. **独立 surface 枚举**：派一个 fresh-context agent（`Agent(general-purpose)`，不传 `model`），prompt = `$ROOT/knowledge/cross-exam/prompts/census.md` 全文 +
   ...
   `ui_surfaces`、`locales`、`axis_support`、`translation_keys`、`dark_variant_gaps` 同样原样入账（顶层同名键），**并把普查官的整份返回原样另存为 run 下
...
3. **改写读回**：每条选中的旅程改写成三元组加 oracle（`done_looks_like` + `stuck_looks_like`），读
   ...
   **`circumstance` 是实例槽位，不是"能跑起来的最小情形"。** 探针自选路径：凡是没被 circumstance
   约束的维度，它一律取**最便宜的那个值**——最短的输入、最空的库、只跑一遍、点第一个、全新状态、
   不等待。这不是随机偏差，是系统性的：取证越便宜越容易完成。于是 circumstance 写成"打开应用"的
   旅程，它的 `done` 读起来像"这份工作做得成"，实际只证明了"在最便宜的那一个点上做得成"，而
   "量大了 / 久了 / 第二次才犯"的毛病结构上进不了证据。改写时对每条旅程问一次"真实使用里这是什么
   情形"，把**决定成败的维度**写进 circumstance——哪些维度决定成败由这份工作自己说了算，技能不给
   清单，也不假设一定是体量。一个点也可以，但要知道它是一个点：**circumstance 覆盖几个点，这条
   旅程的 `done` 就只覆盖几个点**，渲染器按它渲作用域，不许渲成无条件的"走通"。
   **约束情形不破期望隔离**——`circumstance` 与 `preconditions` 约束的是**处境**，oracle 与
   waypoints 约束的是**结果**；处境可以进探针输入，结果绝不能。
4. **落盘**：确认的旅程写入 `journeys[]`，`status: not_examined`，带 `risk`（这份工作的分量 + 若真走
   不通的破坏面）。**未选的候选同样落账**：写进顶层 `journey_candidates[]`（三元组一句话 + 用户
   原话），渲染器列进"未盘问声明"。未选的**面**尚且要写 `not_examined` + `risk`，未选的**旅程**
   直接蒸发是同一个分母纪律上的缺口——报告会说"N 条旅程全部走通"，而没人看得见候选里还有哪些
   压根没盘。`step_budget` 默认 15，按旅程长度调。
...
- **收证据**：把 `steps[]` 和终态截图给用户看（截图给路径；你自己要看时按看图预算取副本，终态整页图先看 overview）。
...
- **派实测官**：`Agent(general-purpose)`（`model` 按 `model_policy`，缺省不传），prompt = `$ROOT/knowledge/cross-exam/prompts/prober.md`
...
- **孤例还是一类（每条 `gap|drift` 落账时必答）**：这个缺口是这一处独有，还是某个结构模式的
  ...
  `$ROOT/knowledge/cross-exam/prompts/sites.md` 全文 + 输入 JSON（target / structure / contract），structure 和
