...
| `state_transition` | 验证状态流转路径 | "订单支付 → 支付成功 → 订单完成" |
| `state_timeout` | 验证超时转换 | "支付超时后自动取消订单" |
| `state_compensation` | 验证补偿路径 | "订单取消后库存恢复" |
...
"ordering_issues": [
  {
    ...
    "seq_pair": [2, 3],
    "description": "seq 2 → seq 3 交接数据「售后申请ID」在上游 task main_flow 中无匹配",
    "severity": "中"
  }
]
```
...
预置脚本保证 schema 一致性和零语法错误。使用 `_common.load_experience_map()` 加载体验地图，通过 `operation_lines[].nodes[].screens[]` 提取界面数据，对每个 screen 使用 `s.get("tasks", [])` 读取任务引用，使用 `_common.get_flow_nodes()` 读取业务流节点（`nodes` 字段，非 `steps`）。

**XV 交叉验证（v3.3.0+）**：脚本自动执行 XV 交叉验证（需 `OPENROUTER_API_KEY` 环境变量）。通过 Python `urllib.request` 直连 OpenRouter API，不依赖 MCP。高严重度边界用例自动追加到用例树，弱验收标准自动标记 `xv_flag`，结果写入 `use-case-tree.json` 的 `cross_model_review` 字段。无 API Key 时静默跳过。
