...
**Step 4: 确认 MCP 已在配置中注册**

检查 MCP 配置是否包含 `stitch` 服务器配置：
- 已有 → 跳过
- 缺失 → 添加到 MCP 配置：
```json
"stitch": {
...
下一步：执行 source ~/.zshrc 后重启，然后执行 setup check 验证连接。
```
...
2. **MCP 配置保护**：对比更新前后的 MCP 配置：
   - 如果原有 server 条目在新版中消失 → **警告用户并自动合并缺失条目**
   - 输出：`⚠️ <plugin>: ai-gateway 条目在新版中缺失，已自动恢复`

3. **汇总报告**：列出每个插件的更新前后版本
