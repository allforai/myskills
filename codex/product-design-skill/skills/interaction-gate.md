...
| 步骤 | 标准模式 | 全自动模式 |
|------|----------|-----------|
| **Step 4 不达标线讨论** | 逐条展示，confirm with user | 自动接受所有不达标线（`disposition: "auto_accepted"`） |

**安全护栏**（自动模式下仍然停下来问用户）：
- ERROR 级验证失败（experience-map.json 解析失败、操作线引用断裂）
- **评分 < 50 的操作线**：总分低于 50 意味着至少两个维度接近零分，属于严重 UX 缺陷，必须停下展示并 confirm with user处置方式，不可 auto_accepted
