...
the snapshot and only enters the identity this way. `exclude` names repo-relative paths that are outside
...
# A git hook exports where *its* repository lives; inherited, they would make every command below
# describe the hook's tree instead of `repo`. The identity is of the tree it was asked about.
HOOK_REPOSITORY_VARS = ('GIT_DIR', 'GIT_WORK_TREE', 'GIT_INDEX_FILE', 'GIT_COMMON_DIR',
                        'GIT_OBJECT_DIRECTORY', 'GIT_ALTERNATE_OBJECT_DIRECTORIES', 'GIT_PREFIX')


def _git(repo, *args):
    """Raw stdout of a git command in `repo`; raises on any failure so the caller can name it."""
    env = {k: v for k, v in os.environ.items() if k not in HOOK_REPOSITORY_VARS}
    return subprocess.run(['git', '-C', str(repo), *args], capture_output=True, check=True, timeout=60,
                          env=env).stdout
...
def _pathspec(exclude):
    """The whole tree minus `exclude`, as a git pathspec."""
    return ['--', '.', *(':(exclude)' + str(p) for p in (exclude or ()))]


def snapshot_digest(repo, exclude=()):
    ...
    h = hashlib.sha256()
    try:
        h.update(_git(repo, 'diff', 'HEAD', '--binary', '--no-color', '--no-ext-diff', *_pathspec(exclude)))
        untracked = _git(repo, 'ls-files', '--others', '--exclude-standard', '-z', *_pathspec(exclude)).split(b'\0')
...
def build_identity(repo, artifacts=(), exclude=()):
    ...
    snapshot, reason = snapshot_digest(repo, exclude)
    if reason:
        out['reason'] = reason
        return out
