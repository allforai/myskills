...
name: superstorm
argument-hint: [goal]
description: Autonomous end-to-end delivery of one large engineering goal (front-loaded decisions, DAG, concurrent worktrees, independent verify). User-invoked only via /superstorm; never invoke it yourself.
---

# Superstorm

> Invoked only by the user as `/superstorm [goal]`. Claude must not start it on its own.
...
This file is the router and contract. For every full Superstorm run, read
