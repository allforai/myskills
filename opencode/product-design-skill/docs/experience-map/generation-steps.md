...
**多模型模式选型（Phase A 之前，consumer_apps 每个 app 执行一次，OpenRouter 可用时必做）**：

> **目的**：单模型判断"连续激励适不适合医疗 App"容易出错。让第二模型**以该领域资深产品经理身份**做模式选型，Claude 拿结果作为 Phase B 设计的基调锚定。
> 这一步的粒度是 **per-app**（不是 per-screen），成本低、杠杆高。

对 consumer_apps 中每个 app，向第二模型发送（并行）：

```
你是 {产品领域} 的资深产品经理，正在设计 {app 名称}（{面向角色}使用的 {平台}端）。

以下是 11 种常见的成熟产品体验模式。请为这个 App 逐项判断：
✅ 适用（说明该 App 中的具体形态）
⚠️ 需调整基调（说明正确基调）
❌ 不适用或有害（说明为什么）
...
调用：`mcp__openrouter__ask_model(task: "pattern_selection_{app}", prompt: 上述)`

**结果注入**：Claude 将第二模型的 pattern 判断写入当前 app 的设计上下文：
- ✅ 的模式 → Phase B 设计时主动考虑
- ⚠️ 的模式 → Phase B 设计时按注明的基调调整（如"完成仪式"的基调从庆祝改为安抚）
- ❌ 的模式 → Phase B 设计时**明确排除**，不设计对应 screen/component

> Claude 自身对第二模型的判断有异议时 → 记录分歧到 pipeline-decisions.json，按更保守的一方执行（宁可少做不错做）。
> OpenRouter 不可用 → Claude 独立判断，参考模式表尾部的适用性原则。
