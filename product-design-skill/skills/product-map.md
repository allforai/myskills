...
| 步骤 | 标准模式 | 全自动模式 |
|------|----------|-----------|
| **Step 0 竞品提问** | AskUserQuestion 询问竞品 | 读 `pipeline_preferences.competitors`：非空 → 直接填充 `competitor-profile.json`，跳过提问；空 → 记录 `competitors: []`，Step 7 Part 3 跳过竞品差异 |
...
| **Step 7 校验** | AskUserQuestion 确认 | ERROR 级问题（必填字段缺失、引用断裂）→ **停下来问用户**；WARNING/INFO → 记日志自动继续 |
...
| 层级 | 字段 | 说明 |
|------|------|------|
| **required** | `id`, `task_name`, `owner_role`, `frequency`, `risk_level`, `main_flow`, `status`, `category` | 任务身份和核心属性，缺失将导致下游技能无法正常工作 |
