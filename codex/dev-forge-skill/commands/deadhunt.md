...
| 能力 | 探测方式 | 重要性 | 降级行为 |
|------|---------|--------|---------|
| Playwright | Playwright browser automation 可用性（任一可用即就绪） | deep/full 必需 | static 模式可跳过；deep/full 阻塞，提示安装 |
...
- **deep/full 模式 + Playwright 未就绪**：输出安装提示并声明推荐选项（「是，帮我安装」/「跳过，仅跑静态分析」/「查看详情」）
