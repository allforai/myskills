"""Synchronize committed, self-contained Claude/Codex evidence engine mirrors.

The mirror is named `engine` and sits beside the visual acceptance mirror (`visual/`) in both cross-exam
ports, and under the scripts directory of both meta-skill ports; a consumer reaches it by path as a sibling
directory, never through sys.path."""
import argparse
from pathlib import Path

SOURCE = Path(__file__).resolve().parent
TARGETS = [SOURCE.parents[1] / 'claude/superstorm/knowledge/cross-exam/engine',
           SOURCE.parents[1] / 'codex/cross-exam-skill/engine',
           SOURCE.parents[1] / 'claude/meta-skill/scripts/engine',
           SOURCE.parents[1] / 'codex/meta-skill/scripts/engine']
SUFFIXES = {'.md', '.py'}
...
if __name__ == '__main__':
    ...
    print('\n'.join(mismatches) if mismatches else 'Evidence engine mirrors match')
