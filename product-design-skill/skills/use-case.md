...
除经典理论外，建议在本技能执行时补充近 12–24 个月的测试设计与需求验证实践：

- 搜索关键词示例：`"risk based testing" + 产品类型 + "case study" + 2025`
- 搜索关键词示例：`"INVEST user stories" + "real project"`
- 来源优先级：官方规范/权威研究 > 一线团队实践 > 社区经验
- 决策留痕：记录 `ADOPT|REJECT|DEFER` 与理由，避免“只读不决策”
...
- 每条关键用例建议补充：`source_refs`、`constraints`、`decision_rationale`。
- `then` 建议覆盖三重成功定义：业务成功 + 技术成功 + 体验成功。
- 高频/高风险用例建议覆盖至少 4/6 视角（`user/business/tech/ux/data/risk`），减少”可执行但不可解释”。

## 跨模型交叉验证（可选增强）

当 `mcp__openrouter__ask_model` 工具可用时，在 Step 3（双格式输出）完成后自动发起交叉验证，结果直接写入产出的 `cross_model_review` 字段。
...
**自动写入内容**：补充边界条件（被遗漏的极端场景、并发条件、时序依赖）、验收标准漏洞（then 条件不可测试、缺少否定断言、缺少数据完整性检查）。
