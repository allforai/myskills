...
### 2.6. data-model-review — 数据模型脑图审核（可选）

> 详见 `${CLAUDE_PLUGIN_ROOT}/commands/data-model-review.md`

可选审核。数据模型（实体、API、VO）作为中间计算产物自动生成，默认不需要单独审核。有两种方式触发：
- 在 wireframe-review 页面点击 "Review Data Model" 按钮（推荐，边审线框边看数据模型）
- 手动运行 `/data-model-review` 命令

适合设计师同时懂技术实现、想在设计阶段验证数据结构的场景。
