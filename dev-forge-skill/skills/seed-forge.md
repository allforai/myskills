...
Step 0: 数据模型映射
      ...
      → Forge 模式: 输出进度「Step 0 模型映射 ✓ {N} 实体, {M} API缺口」（不停）
      → 独立模式: 用户确认映射
      ↓
Step 1: 种子方案设计（核心）
      1-A 角色 → 用户账号设计
      ...
      1-G 用户行为模式设计
      1-H 数据历史深度设计
      → Forge 模式: 输出进度「Step 1 种子方案 ✓ {N} 账号, {M} 实体, {K} 场景链路」（不停）
      → 独立模式: 用户确认方案
      ↓
Step 2: 行业风格与文本模板设定
      名称风格、地区格式、文本模板、图片关键词
      → Forge 模式: 输出进度「Step 2 风格模板 ✓ {industry} 风格, {N} 文本模板」（不停）
      → 独立模式: 用户确认风格
      ↓
  [Forge 编排模式下] Plan 汇总确认:
    ...
    → AskUserQuestion: 确认 / 调整 / 跳过 seed-forge
  ↓
Step 3: 素材采集与图片验收
...
**确认方式**：
- Forge 编排模式 → 不单独确认，汇总到 Plan 汇总确认
- 独立模式 → 用户确认: 映射关系对吗？有遗漏的实体吗？
...
**确认方式**：
- Forge 编排模式 → 不单独确认，汇总到 Plan 汇总确认
- 独立模式 → 用户确认: 数据量合理吗？场景链路对吗？约束规则有遗漏吗？所有枚举值都有覆盖？时间分布和用户行为模式合理吗？
...
**确认方式**：
- Forge 编排模式 → 不单独确认，汇总到 Plan 汇总确认
- 独立模式 → 用户确认: 风格对吗？文本模板覆盖了所有需要填写的文字字段吗？
