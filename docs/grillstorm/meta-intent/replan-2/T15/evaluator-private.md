...
Accepted producer candidate: `ed430dfcebed485b913b54d7785f02cc86998303`, exported as both source and production commit. It supersedes `3f66d7acc367cdb39ee12ae10827c8d81298ae0a`/`88e7becaf30522ef29dd2fa168b62045affcb782`, which carry zero host proof and must not be launched. Source checkout: `/Users/aa/orca/workspaces/myskills/meta-intent-t9`. Coordinator owns fresh actor placement and raw Orca transcript collection. Evaluator owns no production changes and launches no actors.

## Launch and evidence

Run `python3 docs/grillstorm/meta-intent/replan-2/T15/prepare_packets.py <new-absolute-directory> --candidate <accepted-commit>` to export the pinned candidate and fourteen independent projects. The candidate is always named explicitly and never inferred from a checkout HEAD or an installation. The current export is `/private/tmp/meta-intent-T15-ed430dfc`, candidate tree SHA-256 `27d6e62247293dbe1829b48b8741582290d718718c87ddc57eb6bbc26b8e4eb5`. The superseded `/private/tmp/meta-intent-T15-88e7beca` export recorded tree hash `4977a12b...`, which is not reproducible even from its own pinned commit under the current manifest format; treat no previously recorded hash as a candidate fingerprint without recomputing it. Candidate export contains production assets and ADRs but no test source or evaluator documents; it is a temporary test copy, not an installation. The `candidate-manifest.json` fingerprints bytes and symlink targets. Per-cell `source-before.json` fingerprints the synthetic inputs. Do not call these manifests loaded-candidate proof: actual actor reads and raw host records must corroborate the receipt.
...
Before any actor launch, exporter validation found that the first unused packet export lost executable modes (the user-invocation hook was 0644 instead of Git's 0755). `prepare_packets.py` now records exact `git ls-tree` modes under `git_modes`, applies their permission bits to regular files, and records literal symlink targets. Its aggregate fingerprint covers bytes, Git modes and link targets. The public exporter tests were red before these corrections and green afterward. Admission also compares recorded Git modes and link targets to the actual exported filesystem, reporting expected/observed differences and refusing byte-only manifests with missing mode entries. Existing packet directories were not modified, and no host evidence existed. Use the corrected exporter for the coordinator's refreshed accepted candidate; do not launch the original unused snapshot. The exporter now also requires an explicit `--candidate` and refuses to export an implicit one, and the result ledger is guarded by tests that recompute its recorded tree hash from its own pinned commit, refuse a pass without an admitted receipt, and refuse to credit one host's attempt to another. Preparation/helper checks now total twenty tests, still zero real-host cells.
...
The eight `test_admit_evidence.py` cases are explicitly synthetic helper tests and count toward zero host cells. They were developed one red-to-green slice at a time: missing provenance; installed-root/loaded-hash mismatch; changed loaded candidate; changed raw transcript; partial receipt; Git mode/symlink target mismatch. Their synthetic session/transcript strings are not included in any actor project or result attempt. Existing `claude/meta-skill/scripts/capture_evidence.py` can capture actual copied gate commands without adding another command-capture framework. Its wrapper exit code is not the gate result: inspect the stored command `exit_code`.
