...
资产形式：PNG 位图，支持透明通道  
动画形式：帧序列（PNG 序列） 或 Spine 骨骼动画（JSON/binary）  
...
资产形式：Spine 工程文件（.spine）→ 导出 JSON + Atlas PNG  
制作工具：Spine 软件（Esoteric，商业授权）/ DragonBones（开源）  
AI 生图可用性：**不可用**（Spine 工程需人工在 GUI 中绑骨制作）  
AI 可做：生成分层参考图（各骨骼部件独立层），供动画师参考

### VFX 特效

常见形式：
- 帧序列（PNG sequence）：AI + ffmpeg 可生成
- Spine FX：需 Spine GUI，不可自动化
...
- 缓入缓出（Slow In & Out）：动作首尾慢、中间快 → Spine 动画曲线设置
- 弧线运动（Arcs）：有机物体沿弧线运动，而非直线
- 夸张（Exaggeration）：强化动作幅度以增强表达力 → VFX 的尺寸夸张
- **游戏应用**：Spine 动画曲线应遵循缓入缓出；VFX 的消除特效应有压扁-拉伸弹跳
...
Spine 骨骼动画:
  分层 PSD（各部件独立图层）→ Spine 工程（骨骼绑定/动画制作）
  → 导出 JSON + Atlas PNG → Runtime 解析
...
dimension = "2d" AND style ≠ "pixel":
  ...
  → 角色动画方案（frame/spine_lite/spine_full）
  → 特效方案（sprite_sheet/spine_fx/shader_particle）
  → 场景层次（1层/2-3层/多层视差）
  → 概念原画需求（无/角色/场景/两者）
...
| 工具授权成本 | 中（商业授权） | 低（Aseprite 开源可用） |
| 动画师学习曲线 | 高（需学 Spine） | 低（逐帧绘制） |

**推荐规则**：
- 角色数量多、有换装需求 → Spine
- 小团队、无 Spine 授权、像素风 → 帧序列
- VFX 主体特效 → 帧序列；角色附属特效 → Spine FX
...
| 忽略工具链约束 | 选了 Spine Full 但团队没有授权，或选了 Aseprite CLI 但未安装 | Step 1 Q6 明确问工具链约束，写入 config 后节点执行时检查 |
| 低估特效复杂度 | 将 Spine FX 标记为 AI 可生成，实际不可能 | art-tools.md 工具能力矩阵明确标注不可自动化的资产类型 |
