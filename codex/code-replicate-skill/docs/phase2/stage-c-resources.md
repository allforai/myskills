...
| `multi_session` | other_role, other_action, verify_in_current | 多会话真实验证：另一端操作 → 验证当前端收到推送（替代 mock） |

**`multi_session` 详细说明**（Tier 4 — 服务端推送的真实验证）：

> **零 mock 原则**：不 mock WebSocket/SSE 消息，而是真的在另一个浏览器会话中操作，验证当前会话是否实时收到更新。

适用场景：协同编辑、实时通知、多端同步、聊天消息、状态变更推送。
...
```json
{
  ...
  "other_action": [
    {"action": "login"},
    {"action": "navigate", "target": "/orders/ORD-001"},
    {"action": "click", "target": "审批按钮"},
    {"action": "wait", "condition": "审批成功提示"}
  ],
  "verify_in_current": {
    "wait_for": "订单状态标签变化 或 通知弹窗出现",
    "max_wait": "10s",
    "expected_ui_change": "订单状态从'待审批'变为'已审批'，或出现通知'您的订单已被审批'",
    "screenshot": true
  }
}
```
...
**执行协议**：
```
1. 当前 browser context（角色 A）保持在目标页面
2. 新开一个 browser context → 用 other_role 登录
3. 在新 context 中执行 other_action 序列
4. 回到角色 A 的 context → 等待 verify_in_current.wait_for 条件
5. 超时（max_wait）前条件满足 → pass → 截图验证 UI 变化
6. 超时仍未满足 → fail → 记录"实时推送未到达或 UI 未更新"
7. 关闭新 context

测了什么（mock_event_stream 测不到的）：
  ✅ WebSocket/SSE 连接是否建立
  ✅ 后端是否正确推送事件
  ✅ 网络传输是否完整
  ✅ 前端是否正确处理推送并更新 UI
```
...
**`async_seam` 标记**（新增）：当 step 的操作跨越网络接缝（如 click 提交按钮触发 API 调用）时，标记 `"async_seam": true`。LLM 判断依据：该操作的事件处理函数中是否包含 fetch/axios/http 调用。
...
```json
{
  ...
  "async_seam": true,
  "expected_result": {
    "screenshot_during": "按钮变为 loading 状态，表单变灰不可编辑",
    "screenshot_after": "成功跳转到列表页，新记录出现在第一行",
    "ui_changes": ["按钮 loading → 跳转"],
    "feedback": "提交成功"
  }
}
```

`screenshot_during` 描述的是**操作发起到 API 响应之间**的 UI 预期状态。cr-visual 执行时：
- 对 `async_seam: true` 的步骤 → 用 Playwright 慢网模式捕获 `screenshot_during` 对应的过程态
- 对比：过程态是否有 loading 指示？按钮是否 disabled？表单是否锁定？
- 过程态不符合 `screenshot_during` 预期 → 标记 `ASYNC_SEAM_GAP`
...
**流程独立 + 并行优化**：流程间无数据依赖（每个 flow 创建自己的测试数据）。大项目（100+ 截图）不同模块的 flow 可并行执行。同一 flow 内步骤始终串行。
