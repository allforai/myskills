...
1. **按操作线分批**：将所有操作线按角色分组（如 buyer 组、merchant 组、admin 组），每组作为一个批次
2. **每批次独立设计**：LLM 加载该批次涉及的上游数据子集（相关任务、实体、情绪节点），对每个屏幕独立思考后输出设计
...
```python
# batch_fill_{role}.py — 每个屏幕独立设计，不共享模板函数
BATCH_DESIGNS = {
    "S001": {
        "name": "...",
