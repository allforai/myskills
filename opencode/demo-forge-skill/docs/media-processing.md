...
# 4x upscale (significant improvement, slower)
...
# Max file size (200KB, good for avatars)
...
# ImageMagick smart crop (auto subject detection)
...
```bash
# WebP compression (most recommended)
...
# ImageMagick match reference tone
...
```bash
# Standard conversion (H.264, best compatibility)
ffmpeg -i input.mov -c:v libx264 -c:a aac -movflags +faststart output.mp4

# Fast conversion (no re-encode, container swap only)
ffmpeg -i input.mov -c copy output.mp4

# Specify quality (lower CRF = higher quality, 18-28 recommended)
...
```bash
# From 5s, take 30s (no re-encode, fast)
...
```bash
# Rename to IMG-001.webp, IMG-002.webp ...
cd assets/covers
i=1
for f in *.webp; do
...
# Extract and add realesrgan-ncnn-vulkan to PATH
...
# Real-ESRGAN: same as above, download Linux pre-built version
```
