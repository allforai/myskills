...
>
> **模块分节格式**（大型项目强制）：每个模块的 API 设计用 `## Module: {module_name}` 标题分隔。
> 这样下游 Decomposer/Auditor 能按模块名定位对应段落，不需要读全文。
> 示例：`## Module: 认证`、`## Module: 课程管理`、`## Module: 支付`。
...
审查 A — 接口设计审查 (GPT):
  ...
  调用: mcp__ai-gateway__ask_model(task: "structured_output", model_family: "gpt")
  审查: 协议惯例、数据结构一致性、缺失接口、错误响应统一
  输出: issues[] + missing[]
审查 B — 数据模型审查 (DeepSeek):
  提取 design.md 中 ER 设计（Mermaid + 字段定义）
  调用: mcp__ai-gateway__ask_model(task: "technical_validation", model_family: "deepseek")
