# codex/grillstorm-skill/scripts/test_run_layers.py
import json
import pathlib
import subprocess
import tempfile
...
class TestDurabilityAndScope(unittest.TestCase):
    ...
    def test_agent_environment_excludes_ambient_secret(self):
        import os
        os.environ["GRILLSTORM_TEST_SECRET"] = "hidden"
        try:
            template = "/bin/echo {model} {cwd} {out}"
            self.assertNotIn("GRILLSTORM_TEST_SECRET", CodexRunner(template=template).env)
            self.assertEqual(CodexRunner(template=template,
                                         allow_env=["GRILLSTORM_TEST_SECRET"]).env[
                "GRILLSTORM_TEST_SECRET"], "hidden")
        finally:
            os.environ.pop("GRILLSTORM_TEST_SECRET", None)
    ...
    def test_atomic_json_never_leaves_temp(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(list(pathlib.Path(td).glob(".grillstorm-*.tmp")), [])
    ...
    def test_integration_workspace_preserves_dirty_user_tree(self):
        with tempfile.TemporaryDirectory() as root:
            ...
            try:
                ...
                self.assertTrue(ref.startswith("refs/grillstorm/runs/"))
                self.assertTrue(baseline)
            finally:
                subprocess.run(["git", "worktree", "remove", "--force", str(integration)],
                               cwd=root, capture_output=True)

    def test_integration_workspace_recreates_ref_from_portable_commit(self):
        with tempfile.TemporaryDirectory() as root:
            for cmd in (["git", "init", "-q"], ["git", "config", "user.email", "t@t"],
                        ["git", "config", "user.name", "t"],
                        ["git", "commit", "-q", "--allow-empty", "-m", "init"]):
            ...
            subprocess.run(["git", "commit", "-q", "--allow-empty", "-m", "portable"],
                           cwd=root, check=True)
            portable = subprocess.run(
                ["git", "rev-parse", "HEAD"], cwd=root, capture_output=True,
                text=True, check=True).stdout.strip()
            integration, ref, actual_baseline = prepare_integration_workspace(
                pathlib.Path(root), "portable-run",
                {"baseline_commit": baseline,
                 "portable_integration_commit": portable})
            try:
                self.assertEqual(actual_baseline, baseline)
                self.assertEqual(
                    subprocess.run(["git", "rev-parse", ref], cwd=root,
    ...
    def test_portable_resume_rejects_existing_ref_drift(self):
        with tempfile.TemporaryDirectory() as root:
            for cmd in (["git", "init", "-q"], ["git", "config", "user.email", "t@t"],
                        ["git", "config", "user.name", "t"],
                        ["git", "commit", "-q", "--allow-empty", "-m", "init"]):
            ...
            subprocess.run(["git", "update-ref",
                            "refs/grillstorm/runs/drift/integration", baseline],
                           cwd=root, check=True)
            with self.assertRaisesRegex(RuntimeError, "differs"):
                prepare_integration_workspace(
                    pathlib.Path(root), "drift",
                    {"baseline_commit": baseline,
                     "portable_integration_commit": portable})
    ...
    def test_confirmation_marker_recovers_after_snapshot_gap(self):
        with tempfile.TemporaryDirectory() as root:
            ...
            subprocess.run(["git", "commit", "--allow-empty", "-m",
                            "grillstorm-confirmed: T1"], cwd=root,
    ...
    def test_main_publishes_integration_ref_without_touching_user_tree(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertTrue(report["integration_ref"].startswith("refs/grillstorm/runs/"))
