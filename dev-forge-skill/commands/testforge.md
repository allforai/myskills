...
Step B.5: 跨端数据流穿透验证
  ...
  **跨端数据流链的设计方法**（从 business-flows 推导）：

  对每个涉及 ≥2 个子项目的业务实体，构建数据流链：
  ...
  **验证方式（三步穿透）**：
  ...
  1. **写入端操作 + 记录返回值**
     在子项目 A 执行写操作（如用户端注册、创建议题、发起会议），
     记录 API 返回的实体数据（ID、字段值）

  2. **切换到消费端 + 验证数据可见**
     切换到子项目 B（如管理后台），导航到对应页面（如用户列表），
     验证写入端创建的数据**在页面上可见**：
     - 搜索/筛选找到该条数据
     - 验证关键字段值与写入端返回值一致（如邮箱、昵称、状态）
     - 不是检查"页面有数据"，而是检查"写入的那条数据在这里"

  3. **反向穿透**
     消费端执行操作（如管理员禁用用户），切换回写入端验证状态变更生效
     （如被禁用用户无法登录）
  ...
  **数据流链示例**（通用模式，不特化业务域）：
  ...
  | 写入端 | 操作 | 消费端 | 验证 |
  |--------|------|--------|------|
  | 用户端 | 注册账号 | 管理后台 | 用户列表能搜索到该邮箱 |
  | 用户端 | 创建内容 | 管理后台 | 内容管理能看到该条目 |
  | 用户端 | 提交举报 | 管理后台 | 举报队列能看到该举报 |
  | 管理后台 | 禁用用户 | 用户端 | 该用户无法登录或看到禁用提示 |
  | 管理后台 | 修改配置 | 用户端 | 配置生效（如免费额度变更） |
  | 用户 A | 分享内容 | 用户 B | 用户 B 能看到分享的内容 |
