...
5. **注入任务** — 生成共享工具 B1 任务，更新依赖任务的 `_Leverage_` 字段
...
为每个 SU-xxx 生成原子任务，追加到**后端子项目** tasks.md 的 Batch 1 末尾（B1 是 Foundation 层，共享工具属于基础设施）：

```markdown
- [ ] 1.x [api-backend] 创建 {utility_type} 共享工具
  - Files: `src/utils/{name}.ts`, `src/utils/index.ts`
  - 封装 {library}，导出统一接口和类型定义
  - _SharedUtility: SU-001_
  - _Risk: MEDIUM_
```

EU-xxx（复用已有，quality=needs-refactor）时生成重构任务：

```markdown
- [ ] 1.x [api-backend] 重构 {utility_type}（EU-{id}）以符合项目规范
  - Files: `{existing_location}`
  - 补充类型定义和错误处理
  - _SharedUtility: EU-001_
  - _Risk: LOW_
```

### 5b. 更新依赖任务的 `_Leverage_` 字段

扫描所有子项目 tasks.md，找到 `affects_tasks` 中匹配的任务 ID，在其 `_Leverage_` 行追加 SU-xxx 引用：

```
_Leverage: SU-001 (class-validator), SU-003 (ExceptionFilter)_
```

若任务已有 `_Leverage_` 行 → 追加到末尾
若任务无 `_Leverage_` 行 → 在 `_Requirements_` 行后新增
...
各子项目 tasks.md 也会被原地更新（新增 B1 任务 + 更新 _Leverage_ 字段）。
...
Step 4 是唯一的用户交互点。Step 1-3 全自动执行，Step 5 用户确认后全自动完成，不再询问。
