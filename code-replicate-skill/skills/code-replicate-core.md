...
25. **不跳过文件** — LLM 不能根据文件名猜测重要性后跳过文件。Phase 2.3.5 要求对每个模块的未读文件至少读前 20 行（class 定义 + import），发现重要组件后完整读取。每个模块的文件覆盖率必须 ≥ 50%。source-summary 中记录 coverage + files_skipped，用户确认时可见。这个原则对任何项目都适用 — 不靠"必须检查 X 功能"而靠"不能跳过文件"
