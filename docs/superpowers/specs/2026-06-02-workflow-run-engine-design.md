...
5. **runNode** — dispatch the node-spec (+ `profile_slice` + read `decision_inputs`) as a subagent with `NODE_RESULT_SCHEMA`; internal soft-retry loop (≤ `soft_retry_max`, default 2) with progressively stricter prompts; includes `closure_verify` instructions when present. Any mid-run assume+declare write to `assumed-decisions.json` (§5) happens **inside this subagent** (the engine script does no file I/O).
