...
function submitAll(){{
  ...
  fetch('/api/submit',{{method:'POST'}}).then(()=>{{
    ...
    overlay.innerHTML='<div style="font-size:48px">✅</div><div style="font-size:20px;font-weight:600;color:#065f46">Feedback Submitted</div><div style="font-size:14px;color:#64748b">You can close this tab. Run <code>/wireframe-review process</code> to apply.</div>';
...
class WireframeHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/api/feedback":
            ...
            save_feedback(fb)
            self._json({"ok": True})
            print(f"\nWireframe feedback submitted! Saved to {FEEDBACK_PATH}")
            print("Run /wireframe-review process to analyze feedback.")
            import threading
