# Orchestrator Template (Pi)

> Template for generating `.pi/skills/run/SKILL.md` inside target projects.
> Bootstrap reads this template and writes a project-local Pi run skill.
> Pi keeps a markdown loop in that generated skill. It does not invoke the
> Claude Workflow JS engine or Codex `flow.py`. Shared contracts stay on
> `workflow.json` and the copied `scripts/orchestrator` helpers.

## Template: SKILL.md

Below is the Pi-native content that bootstrap should render into `.pi/skills/run/SKILL.md`.

~~~markdown
---
name: run
description: Execute the generated myskills workflow toward a natural-language goal. User-invoked only via /skill:run after bootstrap; never invoke it yourself.
---

# Run — Workflow Orchestrator

User goal: the argument appended as `User: <args>` after `/skill:run`. If that line is missing or empty, reuse the task goal captured during `bootstrap` from `.allforai/bootstrap/bootstrap-profile.json`.
...
through that CLI. Invalid policy blocks for interactive repair. The markdown loop consumes it; execution never opens another interview.
...
- No usable child: execute the node in this session and disclose
  ...
  state. Retry only with the same protocol. Do not switch to `pi -ne`, another
  CLI, or a foreground fallback.
- Asynchronous runs have native completion notices: continue independent
  orchestrator work or return control; merge when notified. Do not poll,
  sleep, or call `bg_wait` only to wait.
- Workers never interview the user. Run Policy and product decisions stay in
  the main session, before the first node.
...
It does not check that the declared repair can actually fix the finding. This loop applies it:
...
the plan; step 1 inside `/skill:run` is a migration for retained history, not a way to open a
...
- For non-blocking safety warnings, consume `--policy-event on_safety_warning`:
  continue logs, halt stops with a report. In this loop, nodes publish a
...
- **A halt is run-wide, and its outputs are quarantined rather than accepted.** Pi
  cannot cancel a worker that has already finished, so the attempt's outputs exist.
...
- `concept-acceptance` names missing behaviour mappings
  ...
  declared QA node whose current report is a positive verdict, this loop routes it to its
...
This generated skill is the driver. Keep looping until a termination condition
below fires. There is no `.allforai/codex/flow.py` and no Claude Workflow JS
engine on Pi. Do not generate either.
~~~
