...
检查 ${CLAUDE_PLUGIN_ROOT}/scripts/gen_experience_map.py 是否存在：
  ...
  不存在 → 用 Read 加载 skills/experience-map.md，按 skill 工作流执行
```
...
自动模式下，Phase 6 优先使用预置脚本（位于 `${CLAUDE_PLUGIN_ROOT}/scripts/`）：
...
| Agent | skill-name | 预置脚本 | --shard 值 | verify --phase | 产出目录 | 分片文件 |
|-------|-----------|---------|-----------|---------------|---------|---------|
| Agent 1 | use-case | `gen_use_cases.py` | `use-case` | `use-case` | `.allforai/use-case/` | `pipeline-decisions-use-case.json` |
| Agent 2 | feature-gap | `gen_feature_gap.py` | `feature-gap` | `feature-gap` | `.allforai/feature-gap/` | `pipeline-decisions-feature-gap.json` |
| Agent 3 | ui-design | `gen_ui_design.py` | `ui-design` | `ui-design` | `.allforai/ui-design/` | `pipeline-decisions-ui-design.json` |
