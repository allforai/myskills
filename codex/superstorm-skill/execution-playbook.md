# Execution Playbook — superstorm pipeline (Codex)
...
silently replace the host permission mode. `SUPERSTORM_CODEX_COMMAND` JSON argv is the
...
5. **Mint the frozen registry (you are the single owner).** Write into the overview,
   wrapped in `<!-- superstorm-registry:start -->` / `<!-- superstorm-registry:end -->`
   (runs started before the rename carry `megastorm-registry` markers; read them the same way)
