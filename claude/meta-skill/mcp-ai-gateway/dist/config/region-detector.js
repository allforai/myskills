...
function getCandidates(models, family) {
...
async function selectBestModelsViaLLM(candidatesByFamily) {
...
Response format: {"gpt":"openai/...","gemini":"google/...","claude":"anthropic/...","deepseek":"deepseek/...","qwen":"qwen/...","llama":"meta-llama/..."}`;
    ...
    const result = await chatCompletion("openai/gpt-4.1-mini", [{ role: "user", content: prompt }], 0);
...
async function writeJsonCache(filePath, data) {
    try {
        const fs = await import("fs");
...
async function getLatestFamilyMap() {
    // In-memory cache
    if (cachedFamilyMap)
        return cachedFamilyMap;
    // File cache
    const diskCache = await readJsonCache(MODEL_CACHE_FILE);
...
function buildRegionMap(familyMap, region) {
    if (region === "global")
        return { ...familyMap };
...
// ---------- Public API ----------
export async function getModelForTask(task, region) {
    const familyMap = await getLatestFamilyMap();
...
export async function getModelFamilyMap(region) {
    const familyMap = await getLatestFamilyMap();
...
export async function getCachedRegion() {
    const cache = await readJsonCache(REGION_CACHE_FILE);
