...
<Split into required (missing or unusable = error return) and optional (missing = warning + continue).>
<Each pull is a seam this node guards — see cross-phase-protocols.md §F: say what makes a required
 value usable for this node, and name the authoritative source when the content exists in more than one place.>
