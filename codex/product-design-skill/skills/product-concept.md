...
2. **向用户确认** 选择题："你描述的核心问题，本质上更接近哪一类？"（基于需求层次生成 2-4 个抽象选项，如"信息不对称"/"流程效率低"/"信任成本高"/"协作断裂"）
...
3. **向用户确认** 选择题（选项基于行为事实，不基于观点预测 — Mom Test 原则）：
   - "这些角色里哪些是你的目标用户？"（多选）
...
10. 为每个实现角色添加 `app` 字段，标注该角色使用的**独立应用**（可部署单元）：
    ...
    **向用户确认**：展示推导出的 role→app 映射，让用户确认或修正。
...
12. **向用户确认**（多选确认）：展示每个角色的操作频度分析和屏幕粒度建议，让用户确认或调整。
...
2. **网络搜索**竞品在这些机制上的做法
3. **向用户确认** 选择题：对每个机制决策点生成 2-4 个选项（基于竞品做法和行业趋势）
...
9. **推导系统边界**：治理风格决定哪些功能在系统内、哪些是外部依赖：
   ...
   - **向用户确认**：「以下功能是在本系统内实现还是依赖外部系统？」（基于治理风格推导的边界建议）
...
> 若只有一个终端用户端，退化为单次提问。多端时合并为一次向用户确认并逐端选择。
...
### 加载校验
- **`concept-decisions.json`**：加载时用 `python -m json.tool` 验证 JSON 合法性。解析失败 → 检查 `.bak` → 提示恢复或重新执行 `product-concept` 工作流。

### 零结果处理
- **Step 0 用户无输入**：若用户未提供任何描述 → 用引导式问题帮助用户补充信息：「请用一句话描述你想做的产品或想解决的问题」，不静默生成空结果。
