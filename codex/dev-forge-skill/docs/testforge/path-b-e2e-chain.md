...
- Path D（integration）+ Path C（platform UI）已完成
...
> **铁律：链内的每一条数据都由链自己通过 UI 操作创建，不依赖任何预置 seed。**

```
禁止：
  ✗ 直接调 API 创建数据（绕过了 UI → API 接缝测试）
...
允许：
  ✓ 通过 UI 表单创建所有需要的数据
  ✓ Chain 0 的真实登录后保存 storageState 复用（唯一允许的捷径）
  ✓ 如果创建前置数据的 UI 流程极长（>10步），可将前置创建封装为 helper 函数
    （helper 内部仍然走 UI 操作，不是 API 调用）

数据隔离：
  - 每条链用唯一前缀/后缀标记数据（时间戳或 UUID 片段）
  - 链结束后不清理数据（下次运行创建新数据，不依赖旧数据不存在）
