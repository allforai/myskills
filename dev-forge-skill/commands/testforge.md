...
| experience-priority | `.allforai/product-map/product-map.json` → `experience_priority` | 判断是否要追加用户端成熟度测试要求 |
...
若 `experience_priority.mode = consumer | mixed`，额外推导：
- 用户完成一次操作后，不知道下一步做什么？
- 结果回流失败：提交成功但首页/列表/历史未反映
- 持续关系断裂：提醒/通知/进度入口存在但无内容或无法进入后续链路
...
4. 链路设计原则（通用）
   ...
   - Given-When-Then 分离：
     - **Given（前置）用 API 准备**（curl 创建商品/用户/配置，快且稳定）
     - **When（操作）用 UI 执行**（这才是被测目标）
     - **Then（验证）用 API + UI 双重验证**（API 确认数据正确，UI 确认显示正确）
   - 失败时截图/日志以便调试
```
...
Step B.5: 跨端数据流穿透验证
  ...
  **Step B.5.2: 逐链执行穿透验证**
  ...
  ```
  数据流链 = 前置准备(API) → 写入操作(子项目A) → 等待+刷新 → 消费端验证(子项目B) → 反向穿透
  ```
  ...
  **1. 写入端操作 + 记录唯一标识**
     ...
     - **Playwright（Web）**: 拦截 network response 提取 API 返回值
     - **Flutter integration_test**: 从 UI 上读取显示的唯一标识（如订单号、ID）
       或在操作后 curl API 查询最新创建的记录
     - **Maestro/XCUITest**: 从 UI 截图/文本提取，或 curl API 查询
     - **通用回退**: 操作前后各 curl 一次列表 API，diff 找到新增记录

  **2. 等待 + 刷新消费端（关键步骤，不可省略）**
     写入端操作完成后，消费端不一定能立即看到数据（异步处理/缓存/无 WebSocket）。
     必须在消费端验证前执行：
     - **Web**: `page.reload()` 或等待 + 重新导航到目标页面
     - **Flutter**: pull-to-refresh 手势（`tester.fling` 下拉）或重新 push 路由
     - **通用**: `sleep 2s` + 刷新页面（粗暴但有效）
     - 如果项目有 WebSocket/SSE 实时推送 → 等待推送事件到达（最多 5s），无需手动刷新
     **不刷新直接验证 → 大概率假阴性（数据在但页面没更新）**

  **3. 消费端验证（强断言）**
     切换到消费端子项目，导航到对应页面，
     用 Step 1 记录的唯一标识验证**该条数据在页面上可见且正确**：
     - 搜索/筛选找到该条数据（用 ID 或唯一字段值）
     - 验证关键字段值与写入端一致（不只是存在，而是值对）
     - **不是"页面有数据"，而是"我刚写入的那条数据在这里且值正确"**

  **4. 反向穿透**（如果有反向流）
     消费端执行操作后（如商户确认），同样 **等待+刷新** 写入端（如消费者端），
     验证状态变更在写入端生效
...
**后端环境要求**：E2E 测试时，后端的认证相关 dev bypass 必须关闭。E2E 测的是"真实用户使用的系统"，不是"开发者调试用的系统"。如果后端有 dev mode 认证绕过，E2E 启动后端时必须确保该绕过不生效（关闭 dev auth bypass 或设置环境变量禁用）。其他 dev 便利（如 mock LLM、mock 支付）可以保留——E2E 关注的是认证和数据链路的真实性，不要求所有外部服务都真实。
