...
argument-hint: "[mode: static|deep|full|incremental]"
...
- **`deep`** → 仅深度测试：Phase 0 → Phase 2 → Phase 3（根据项目平台自动选择 E2E 工具）→ Phase 4 → Phase 5
...
Phase 3 深度测试依赖 E2E 工具，Phase 0 检测可用性：

| 能力 | 探测方式 | 重要性 | 降级行为 |
|------|---------|--------|---------|
| E2E 测试工具 | **LLM 推理**：根据项目平台和技术栈，检测可用的 E2E 工具（MCP 工具列表 + `which` CLI 探测 + 项目依赖声明） | deep/full 必需 | static 模式可跳过；deep/full 无可用工具时阻塞，提示安装 |

**输出格式**：

```
外部能力:
  E2E 工具  {✓/✗} {工具名}   深度测试（Phase 3）
```

**交互式安装引导**（统一协议见 `product-design-skill/docs/skill-commons.md`）：

- **static 模式**：不需要 E2E 工具，不阻塞
- **deep/full 模式 + E2E 工具未就绪**：用 AskUserQuestion 提供安装建议（根据项目技术栈推荐最合适的工具，如「是，帮我安装」/「跳过，仅跑静态分析」/「查看详情」）

---

## 详细文档（按需用 Read 工具加载）

- `${CLAUDE_PLUGIN_ROOT}/docs/deadhunt/quick-start.md` — 快速开始
- `${CLAUDE_PLUGIN_ROOT}/docs/deadhunt/workflow.md` — 工作流详解
- `${CLAUDE_PLUGIN_ROOT}/docs/deadhunt/phase0-analyze.md` — Phase 0: 项目分析
- `${CLAUDE_PLUGIN_ROOT}/docs/deadhunt/auth-strategy.md` — 登录认证策略
- `${CLAUDE_PLUGIN_ROOT}/docs/deadhunt/phase1-static.md` — Phase 1: 静态分析
- `${CLAUDE_PLUGIN_ROOT}/docs/deadhunt/phase2-plan.md` — Phase 2: 测试计划
- `${CLAUDE_PLUGIN_ROOT}/docs/deadhunt/phase3-test.md` — Phase 3: 深度测试（索引文件，指向子文件）
- `${CLAUDE_PLUGIN_ROOT}/docs/deadhunt/phase3/overview.md` — Phase 3: 性能优化与分级策略
- `${CLAUDE_PLUGIN_ROOT}/docs/deadhunt/phase3/404-scanner.md` — Phase 3: 全局 404 监控 + Layer 1-5
- `${CLAUDE_PLUGIN_ROOT}/docs/deadhunt/phase3/intent-classification.md` — Phase 3: 死链意图判定
- `${CLAUDE_PLUGIN_ROOT}/docs/deadhunt/phase3/validation.md` — Phase 3: CRUD 闭环 + 数据展示 + 业务流程
- `${CLAUDE_PLUGIN_ROOT}/docs/deadhunt/phase3/convergence.md` — Phase 3: 多轮收敛机制
- `${CLAUDE_PLUGIN_ROOT}/docs/deadhunt/phase3/patrol.md` — Phase 3: Patrol 引擎（Flutter）
- `${CLAUDE_PLUGIN_ROOT}/docs/deadhunt/phase4-report.md` — Phase 4: 报告生成
- `${CLAUDE_PLUGIN_ROOT}/docs/deadhunt/phase5-supplement-test.md` — Phase 5: 补充测试
- `${CLAUDE_PLUGIN_ROOT}/docs/deadhunt/faq.md` — FAQ
...
> 详见 `${CLAUDE_PLUGIN_ROOT}/docs/deadhunt/phase5-supplement-test.md`
...
**测试文件命名：**
- `deadhunt-regression.{ext}` — 回归测试（扩展名按项目测试框架惯例）
- `deadhunt-coverage.{ext}` — 覆盖盲区测试（扩展名按项目测试框架惯例）
- LLM 根据项目技术栈自行决定文件格式和命名风格
