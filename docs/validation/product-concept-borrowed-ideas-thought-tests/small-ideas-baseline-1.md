...
| 条目 | 借来的做法 | 场景 | 实测 | 结论 |
|---|---|---|---|---|
| A | 说"以后再做"必须写明**什么情况发生就重新捡起来** | 独立开票工具 3 周裁剪范围（`feature-prune.md`） | 两遍都把"缓"和"砍"在文字里分开了（"本次对 T04–T09 是延后而非砍掉，未来版本重新进入 gap"；"T05 待海外需求出现再开"），**但没有逐项写出可观察的重开条件**——因为没有地方写 | **有用但用户决定不改**（缺一个字段；2026-09-21 用户明确「不改」，本条记为不采纳，证据留档） |
