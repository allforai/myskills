...
**先读后评（所有维度共用原则）**：
LLM 对每个产物条目评分时，必须**实际读了目标代码**才能判定匹配，不能只 Grep 文件名/类名就给分。

评分流程：
1. **Read**：通过 fidelity-index 定位目标文件 → 打开读取相关代码段
2. **理解**：确认代码做了什么（不是"文件存在"而是"逻辑正确"）
3. **评分**：基于理解给出 match / gap + evidence
