#!/usr/bin/env python3
"""superstorm §1.6 deterministic execution runner (Codex port).
...
Usage:
  python3 run_layers.py orchestration.json all-tasks.json \
      ...
      [--state .superstorm-state.json] [--report execution-report.json] \
...
class CodexRunner:
    ...
    def run(self, prompt, model, cwd, *, role="executor", task_id="standalone",
        ...
        scratch = pathlib.Path(tempfile.mkdtemp(prefix=f"superstorm-scratch-{task_id}-"))
...
def run_task_in_worktree(task, runner, models, root, prompts_dir, merge_lock, log=print,
                         branch_prefix="superstorm", enforce_paths=False,
    ...
    with merge_lock:
        ...
        safety_commit(root, f"superstorm: pre-worktree snapshot before {tid}")
    ...
    try:
        ...
        safety_commit(wt, f"superstorm: {tid}")
        marker = ("superstorm-reality-gated" if result["status"] == "reality_gated"
                  else "superstorm-confirmed")
        ...
        with merge_lock:  # integration ref publication and worktree reset serialize
            safety_commit(root, f"superstorm: integration snapshot before admitting {tid}")
...
def atomic_write_json(path, value):
    ...
    fd, tmp = tempfile.mkstemp(prefix=".superstorm-", suffix=".tmp", dir=parent)
    try:
...
def prepare_integration_workspace(repo, run_id, prior_state):
    ...
    ref = f"refs/superstorm/runs/{run_id}/integration"
    baseline_ref = f"refs/superstorm/runs/{run_id}/baseline"
    ...
    run_root = pathlib.Path(tempfile.mkdtemp(prefix=f"superstorm-{run_id[:8]}-"))
...
def recover_confirmed_from_git(integration, baseline, task_ids):
    ...
    for subject in history.stdout.splitlines():
        for prefix, status in (("superstorm-confirmed: ", "done"),
                               ("superstorm-reality-gated: ", "reality_gated")):
...
def main(argv):
    ...
    ap.add_argument("--state", default=".superstorm-state.json")
    ...
    def run_isolated(task):
        result = run_task_in_worktree(
            ...
            branch_prefix=f"superstorm-{run_id[:8]}",
