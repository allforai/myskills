...
| `architecture_pattern = library-sdk` or `cli` | `user_steps` `/product-review` (no UI, no product to critique) | `user_steps: ["/cross-exam"]` — a CLI or library is still a delivery whose completion is examined; the summary states the exemption |
