...
> - Phase 6（demo-forge）为独立插件 `demo-forge-skill/`，提示用户执行 `demo-forge design` 工作流。
...
- 不存在 → 输出「请先完成产品设计流程（执行 `product-design full` 工作流），再运行项目锻造」，终止
...
- **OpenRouter 未就绪**：提示执行 setup 工作流配置（Key 存储在 MCP 配置中，不污染 shell 环境变量），不阻塞
...
各 spike 类别互不依赖，并行执行调研。分派 N 个子任务（N = 检测到的 spike 数量），每个子任务负责一个 spike 类别的 2-3 轮网络搜索 + 方案提炼。
...
- 可以稍后补充（执行 project-forge credentials 更新）
```
...
design-to-spec 内部使用并行执行加速：后端子项目先完成 spec，然后多个前端子项目并行生成 spec。详见 design-to-spec.md 的「并行执行编排」段落。
...
Step 4: 执行修复
  调用 `task-execute` 工作流执行 Fix Round
...
代码稳定后，提示用户执行 `demo-forge design` 工作流完成演示数据方案设计。

> 注: demo-forge 已独立为 `demo-forge-skill/` 插件。本阶段仅做 plan 设计，完整灌入（media + execute + verify）在最终报告后执行 `demo-forge` 工作流。
