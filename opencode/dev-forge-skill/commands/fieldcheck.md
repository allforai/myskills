...
1. 用 Read 工具读取 `${CLAUDE_PLUGIN_ROOT}/docs/deadhunt/fieldcheck/overview.md` 获取完整执行流程和注意事项
...
- `${CLAUDE_PLUGIN_ROOT}/docs/deadhunt/fieldcheck/overview.md` — 执行流程总览、问题分类、注意事项
- `${CLAUDE_PLUGIN_ROOT}/docs/deadhunt/fieldcheck/extractors.md` — 各层字段提取方法详解（含所有技术栈）
- `${CLAUDE_PLUGIN_ROOT}/docs/deadhunt/fieldcheck/matching.md` — 智能匹配算法详解
- `${CLAUDE_PLUGIN_ROOT}/docs/deadhunt/fieldcheck/report.md` — 报告格式定义和 JSON schema
...
> **Flutter 项目注意**：当检测到 Flutter 客户端时，L1 提取会扫描 `.dart` Widget 文件，L2 提取会扫描 Dart 模型类（`fromJson`/`@JsonKey`）。同时启用 Flutter 特有问题类型检测（PLATFORM_GAP、SERIALIZE_MISMATCH、NULL_SAFETY）。详见 `${CLAUDE_PLUGIN_ROOT}/docs/deadhunt/fieldcheck/extractors.md`。
