#!/usr/bin/env python3
"""Data Model Review Server — interactive mind map for reviewing entity models,
API contracts, and view objects.

Transforms entity-model.json, api-contracts.json, and view-objects.json into
an interactive radial mind map. Users can expand/collapse branches, click nodes
to add comments, and approve or flag nodes for revision.

Output: .allforai/data-model-review/review-feedback.json

Usage:
    python3 datamodel_review_server.py <BASE_PATH> [--port 18904] [--no-open true]
"""
...
SOURCE = "data-model"
PORT = int(args.get("port", "18904"))
HOST = args.get("host", "localhost")
NO_OPEN = args.get("no-open", "false").lower() in ("true", "1", "yes")

REVIEW_DIR = os.path.join(BASE, "data-model-review")
...
.mm-node[data-type="entity"] .label{{border:2px solid #1976D2;background:#E3F2FD;color:#1565C0;font-weight:600}}
.mm-node[data-type="field-group"] .label{{background:#F5F5F5;color:#616161;font-weight:600;font-size:12px}}
.mm-node[data-type="field"] .label{{background:#FFF;color:#424242;font-weight:400;font-size:12px}}
.mm-node[data-type="field-pk"] .label{{border:2px solid #FFA000;background:#FFF8E1;color:#E65100;font-weight:500;font-size:12px}}
.mm-node[data-type="field-required"] .label{{background:#FFF;color:#C62828;font-weight:400;font-size:12px}}
.mm-node[data-type="state-machine"] .label{{background:#E0F7FA;color:#00695C;font-weight:600;font-size:12px}}
.mm-node[data-type="transition"] .label{{background:#E0F7FA;color:#00695C;font-weight:400;font-size:12px}}
.mm-node[data-type="api-group"] .label{{background:#E8F5E9;color:#2E7D32;font-weight:600;font-size:12px}}
.mm-node[data-type="api"] .label{{background:#E8F5E9;color:#1B5E20;font-weight:400;font-size:12px}}
.mm-node[data-type="vo-group"] .label{{background:#F3E5F5;color:#7B1FA2;font-weight:600;font-size:12px}}
.mm-node[data-type="vo"] .label{{background:#F3E5F5;color:#6A1B9A;font-weight:400;font-size:12px}}
.mm-node[data-type="action"] .label{{background:#FFF3E0;color:#E65100;font-weight:400;font-size:12px}}
.mm-node[data-type="relation-group"] .label{{background:#FBE9E7;color:#BF360C;font-weight:600;font-size:12px}}
.mm-node[data-type="relation"] .label{{background:#FBE9E7;color:#BF360C;font-weight:400;font-size:12px}}
.mm-node[data-type="error"] .label{{background:#fef2f2;color:#dc2626;border-color:#fca5a5}}
/* Method color badges */
.method-badge{{display:inline-block;font-size:9px;font-weight:700;padding:1px 5px;border-radius:3px;margin-right:4px;line-height:1.4}}
.method-GET{{background:#dcfce7;color:#15803d}}
.method-POST{{background:#dbeafe;color:#1d4ed8}}
.method-PUT{{background:#fed7aa;color:#c2410c}}
.method-PATCH{{background:#ccfbf1;color:#0f766e}}
.method-DELETE{{background:#fecaca;color:#dc2626}}
/* Tags */
.tag-list{{background:#dcfce7;color:#15803d}}
.tag-create{{background:#dbeafe;color:#1d4ed8}}
.tag-detail,.tag-read{{background:#e0f2fe;color:#0369a1}}
.tag-update{{background:#fed7aa;color:#c2410c}}
.tag-delete{{background:#fecaca;color:#dc2626}}
.tag-state{{background:#ccfbf1;color:#0f766e}}
/* Collapse indicator */
...
.cp-header .cp-detail{{font-size:12px;color:#64748b;white-space:pre-line}}
...
<div class="comment-panel" id="commentPanel">
  ...
  <div class="cp-body">
    ...
    <select id="commentCategory">
      <option value="entity">实体定义问题</option>
      <option value="api">接口设计问题</option>
      <option value="vo">视图对象问题</option>
      <option value="action">Action Binding 问题</option>
      <option value="state-machine">状态机问题</option>
      <option value="product-map">根源性问题</option>
    </select>
...
function submitAll(){{
  if(!confirm('Submit data model review feedback?'))return;
...
class DataModelReviewHandler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
...
def main():
    ...
    server = http.server.HTTPServer((HOST, PORT), DataModelReviewHandler)
