...
run: `2026-09-18-product-experience-overhaul` · 分支 `product-experience-overhaul` · 被测 HEAD `d28f7b4c`
基线: `6a59fe44`（`test(pi): 补宿主侧核查——模型解析链与外部 CLI 的实际模型归属`）——1198 passed / 4 failed（meta-skill 全量），superstorm 255 passed
执行: 2026-09-18，逐条串行，一次只跑一个。解释器 `python3` = CPython 3.14（`/opt/homebrew/opt/python@3.14`），pytest 9.0.3。
原始输出: `docs/superpowers/runs/2026-09-18-product-experience-overhaul/logs/T-M5-14.log`

## 逐条结果

R-M5-05 的九条（C1–C9）+ 规格"验收"两条（A1–A2）+ 两条跨模块不变量（I1–I2）。

| # | 命令 | 退出码 | passed / failed | 耗时 |
|---|---|---|---|---|
| C1 | `python3 -m pytest -q -rfE claude/meta-skill/tests/unit` | 1 | 1297 passed / 4 failed | 358.8s |
| C2 | `python3 -m pytest -q claude/superstorm/scripts` | 0 | 255 passed / 0 failed | 6.6s |
| C3 | `python3 -m pytest -q codex/cross-exam-skill/scripts` | 0 | 172 passed / 0 failed | 6.8s |
| C4 | `python3 -m pytest -q shared/evidence-engine` | 0 | 46 passed / 0 failed | 3.7s |
| C5 | `python3 -m pytest -q shared/visual-acceptance` | 0 | 211 passed / 0 failed | 3.5s |
| C6 | `python3 -m pytest -q shared/scripts/orchestrator shared/keep-code-simple pi/cross-exam` | 2 | 0 collected / 1 collection error | 0.3s |
| C6a | `python3 -m pytest -q shared/scripts/orchestrator` | 0 | 144 passed / 0 failed | 11.4s |
| C6b | `python3 -m pytest -q shared/keep-code-simple` | 0 | 14 passed（+40 subtests）/ 0 failed | 0.3s |
| C6c | `python3 -m pytest -q pi/cross-exam` | 0 | 7 passed / 0 failed | 0.2s |
| C7 | `python3 -m pytest -q pi/meta-skill/test_contract.py codex/meta-skill/test_flow.py codex/meta-skill/test_install.py` | 0 | 202 passed（+21 subtests）/ 0 failed | 43.9s |
...
C6 是命令形状缺陷，不是代码回归；见下节。C6a/C6b/C6c 是同三个目录按仓库惯例分次调用的实际结果。

## meta-skill 全量：失败用例 id 与基线逐一比对

C1 用 `-rfE` 取到的失败/错误 node id 集合，共 4 个，无收集错误：
...
失败集合 == 基线四个，`失败集合 ⊆ 基线四个` 成立；第五个失败不存在。四个用例本轮未被触碰。
...
`python3 -m pytest -q shared/scripts/orchestrator shared/keep-code-simple pi/cross-exam` 在收集阶段就中断：
...
- 两个文件在基线 `6a59fe44` 就都已存在（`git ls-tree 6a59fe44`），本轮未改动任何一个，交换两个目录的顺序
  报错也对称出现（先收集到哪个，另一个就报 mismatch）；清掉 `__pycache__` 后复现依旧，是确定性冲突。
...
**建议的命令订正**（R-M5-05 第六条 / T-M5-14 验收串中的对应一段）：
...
订正只改调用形状，不改任何被测文件；不订正则该条恒为退出码 2，与被测代码无关。

## 结论

- meta-skill 全量失败集合 == 基线四个，通过数 1297 ≥ 1198，无收集错误。
- 其余套件与两条不变量全部符合预期；C6 按仓库惯例分次调用后同样全绿。
