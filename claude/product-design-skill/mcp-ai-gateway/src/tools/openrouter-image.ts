...
export function registerOpenRouterImage(server: McpServer): void {
  server.tool(
    "openrouter_generate_image",
    "Generate image via OpenRouter using GPT-5 Image or Gemini Image models. Uses existing OpenRouter key, no extra API key needed.",
    ...
    async (params) => {
      try {
        ...
        const body = (await res.json()) as { choices: ImageChoice[] };
