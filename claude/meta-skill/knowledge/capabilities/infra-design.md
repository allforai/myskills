...
**Vercel deployment** (`bootstrap-profile.json.deployment_platform == 'vercel'`): Vercel IS the CDN, serverless runtime, and deployment platform. Suppress infra-design entirely — no interactive dimension questions needed. Create a minimal `infra-design.json` documenting: Vercel Edge Network (CDN), Vercel Serverless Functions (API routes), and any additional services (e.g., Vercel KV, Vercel Postgres, Vercel Blob if used). Document env var requirements from `.env.example`.
...
- PocketBase (`baas-pocketbase`): Single Go binary with embedded SQLite, REST API, realtime SSE (no WebSocket), Auth, File storage. Local: `pocketbase serve` (starts on `localhost:8090`). Note: PocketBase uses its own Admin UI for schema management — there is no migration file system or ORM. Configuration: `POCKETBASE_URL`, `PB_ADMIN_EMAIL`, `PB_ADMIN_PASSWORD`. Scaling: PocketBase is single-process; for high availability, use PocketBase with Litestream for SQLite replication. Capture schema as `pb_schema.json` export from Admin API.
