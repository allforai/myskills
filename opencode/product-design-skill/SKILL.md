...
phases:
  ...
  - id: experience-map
    ...
    rules: ["./skills/experience-map.md"]
...
> **Stitch 决策点**（Phase 5.5）：review 线框 tab 通过后、进入 Phase 6-8 并行执行前，检查 Stitch MCP 可用性。不可用时 向用户提问 三选一（上传设计稿 / 跳过视觉验收 / 配置 Stitch）。选择跳过时记入 pipeline-decisions（`stitch_skipped`），design-audit 标记 `stitch_skipped: true`。
...
`../../shared/scripts/product-design/` 中保留的脚本仅用于**验证、聚合和 product-map 子产物生成**。所有创意/判断性内容（情绪标注、界面设计、交互评分、缺口分析、用例推导、UI 设计）均由 LLM 主导生成。
