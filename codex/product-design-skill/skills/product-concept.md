...
用户输入（可以很模糊）
  ...
  → 提炼为 2-4 个 present choices to the user
  → 用户选择
  → 基于选择结果，再次搜索更精准的信息
  → 范围缩小，进入下一轮或下一 Step
```
...
2. **基于用户新输入网络搜索** — 以用户提供的关键词为线索重新搜索
...
**搜索关键词**：`”JTBD” + 行业词 + “case study” + 2025`、`”problem discovery” + 产品类型 + “user research”`、`”Blue Ocean” + 行业词 + “competitive landscape”`
...
2. **confirm with user** 选择题："你描述的核心问题，本质上更接近哪一类？"（基于需求层次生成 2-4 个抽象选项，如"信息不对称"/"流程效率低"/"信任成本高"/"协作断裂"）
...
**present choices to the user**（假设清零后）：
...
3. **confirm with user** 选择题（选项基于行为事实，不基于观点预测 — Mom Test 原则）：
   - "这些角色里哪些是你的目标用户？"（多选）
...
10. 为每个实现角色添加 `app` 字段，标注该角色使用的**独立应用**（可部署单元）：
    ...
    **confirm with user**：展示推导出的 role→app 映射，让用户确认或修正。
...
12. **confirm with user**（多选确认）：展示每个角色的操作频度分析和屏幕粒度建议，让用户确认或调整。
...
2. **网络搜索**竞品在这些机制上的做法
3. **confirm with user** 选择题：对每个机制决策点生成 2-4 个选项（基于竞品做法和行业趋势）
...
8. **对每个业务流，present choices to the user**：
...
9. **推导系统边界**：治理风格决定哪些功能在系统内、哪些是外部依赖：
   ...
   - **confirm with user** 确认：「以下功能是在本系统内实现还是依赖外部系统？」（基于治理风格推导的边界建议）
...
**present choices to the user**（对抗性生成后）：
...
对每个终端用户端类型，present a single-choice question：
...
> 若只有一个终端用户端，退化为单次提问。多端时合并为一次 confirm with user 逐端选择。

**Q2 — 竞品参考**

present a question with preset options：
...
检测 Stitch MCP 可用性：
- 检查 MCP 工具 `Stitch MCP create_project` 是否可用
...
present a multi-choice question：
...
- 基于反推结果 **网络搜索**竞品和行业趋势
- **confirm with user**：验证反推的问题域、角色、价值是否准确
- 补充商业模式层（代码中通常没有）
...
### 零结果处理
- **Step 0 用户无输入**：若用户未提供任何描述 → 用 guide the user with choices：「请用一句话描述你想做的产品或想解决的问题」，不静默生成空结果。
- **网络搜索连续 3 轮无有用结果**：切换为纯访谈模式 — 基于 AI 知识 + 用户输入推进，标记 `evidence_mode: "interview_only"`，告知用户「公开信息较少，已切换为访谈模式」。
...
### 网络搜索不可用
- **工具不可用**：告知用户「⚠ 网络搜索暂不可用」→ 提供选项：(a) 切换为纯访谈模式（基于 AI 知识 + 用户输入），标记 `evidence_mode: "interview_only"` (b) 用户手动提供参考资料。
...
所有问题都基于搜索结果生成选项。用户只需选择，不需要从零思考。如果用户选择"其他"并提供自定义输入，必须走「其他响应处理流程」：基于新输入网络搜索 → 生成新选择题 → 用户从新选项中确认。不可直接采纳未经搜索验证的自由输入。

### 2. 搜索先行，选项后生

每轮提问前必须先 网络搜索互联网。选项必须基于搜索结果生成，不可凭空编造。搜索关键词从用户已确认的内容中提取，确保每轮搜索都更精准。
