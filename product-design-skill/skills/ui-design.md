...
2. 检测 Stitch MCP 可用性：
   ...
   - **不可用 → 必须使用 AskUserQuestion 询问用户（禁止静默降级）**：

     **「Stitch UI 视觉稿服务未就绪。如何处理？」**

     选项：
     - **跳过视觉稿** — 仅生成 stitch-prompts.json（可稍后手动使用）
     - **先去配置** — 中断当前流程，运行 `/setup` 配置 Stitch 后重新执行 `/ui-design`

     用户选择「跳过视觉稿」→ 跳转 3b
     用户选择「先去配置」→ **立即停止当前 Step 5.5**，提示用户运行 `/setup`
