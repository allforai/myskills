...
description: >
  ...
  Pipeline: concept → concept-review → map → map-review → data-model-review → experience-map → wireframe-review (structure lock) →
...
### 2.6. data-model-review — 数据模型脑图审核

> 详见 `${CLAUDE_PLUGIN_ROOT}/commands/data-model-review.md`

product-map Step 7/8 完成后，启动数据模型审核服务器，以树形结构展示实体、字段、状态机、API 接口、视图对象，用户可标注评论。

```
/data-model-review          # 启动审核服务器（localhost:18904）
/data-model-review process  # 读取反馈，汇总修改建议
```
...
启动低保真线框审核服务器，使用交互类型模板渲染差异化线框（列表/表单/详情/状态机/审批等），提供 4D 面板（Data/Action/State/Flow）和 6V 视角（Structure/Behavior/Data/State/Flow/Emotion）。如果配置了 OPENROUTER_API_KEY，自动执行 XV 跨模型校验。反馈按类别路由到上游（product-map / experience-map / concept），审核通过后结构锁定。
...
Phase 2-9 的纯转换脚本已预置到 `${CLAUDE_PLUGIN_ROOT}/scripts/`，随插件部署：

| 脚本 | Phase | 关键修复 |
|------|-------|---------|
| `_common.py` | 共享 | 字段常量、数据加载器、pipeline-decisions 去重写入 |
| `gen_business_flows.py` | 2 product-map Step 3 | 业务流候选生成、交接验证、孤立/幽灵状态检测、泳道图 |
| `gen_product_map.py` | 2 product-map Step 6 | 聚合角色+任务+业务流+冲突+约束，生成 task-index/flow-index |
| `gen_validation_report.py` | 2 product-map Step 7 | 完整性扫描 + 冲突复查，输出 validation-report |
