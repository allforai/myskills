...
- **Level content pipeline rule:** When `puzzle-design` is selected AND the design spec indicates a level count that humans cannot realistically author by hand (detected from `level_count`, chapter count × levels-per-chapter, or explicit "procedural"/"generated" signals in design docs): generate `level-content-pipeline` instead of (or in addition to) a GUI `level-editor-tool` node. The `level-content-pipeline` node-spec MUST include automated generation, a genre-appropriate solver/validator that rejects unsolvable or trivially easy levels, and a generation loop that writes accepted level files to the game client. A GUI editor MAY be included as a secondary tool for human spot-check. Consult `knowledge/domains/game-genre-specialization.md` for the genre-specific solver pattern that matches the detected puzzle type.
