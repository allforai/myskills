...
"protocol_spec": {
  "frame_format": [
    {"field": "magic", "offset": 0, "length": 2, "encoding": "uint16_be", "value": "0xABCD"},
    {"field": "payload_length", "offset": 2, "length": 4, "encoding": "uint32_be"},
    {"field": "payload", "offset": 6, "length": "variable", "encoding": "rabbit_encrypted"},
    {"field": "checksum", "offset": -4, "length": 4, "encoding": "crc32"}
  ],
  "state_machine": [
    {"state": "HANDSHAKE", "on": "server_hello", "next": "KEY_EXCHANGE"},
    {"state": "KEY_EXCHANGE", "on": "key_accepted", "next": "READY"},
    {"state": "READY", "on": "data_frame", "next": "READY"}
  ],
  "test_vectors": [
    {"input_hex": "ABCD00000006...", "expected_output": "decrypted payload bytes", "description": "basic frame parse"}
  ]
}
```

`protocol_spec` 不是文本描述，是**可验证的结构化规格**。`test_vectors` 从源码的单元测试或常量中提取，供目标代码验证行为一致性
...
```json
{
  ...
  "infrastructure_sources": [
    {"file": "lib/net/ws_tunnel.dart", "how": "WebSocket Tunnel 实现：自定义帧协议 + Rabbit 加密层"},
    {"file": "native/ios/CDNSDK.framework", "how": "CDN 加速 SDK，预编译二进制，无源码"},
    {"file": "lib/crypto/rabbit.dart", "how": "Rabbit 流密码实现，不可替换为 AES"}
  ],
  "dependency_map": [
...
LLM 读 infrastructure-profile 中 `cannot_substitute: true` 或 `compatibility: exact` 的组件的源码实现，提取**可验证的输入输出对**：
  ...
  "vectors": [
    {
      ...
      "cases": [
        {"input": "base64 encoded plaintext", "key": "base64 key", "expected_output": "base64 ciphertext",
         "description": "encrypt 'hello world' with test key"}
      ]
    },
    {
      "component": "WebSocket Tunnel framing",
      "source_file": "lib/net/ws_tunnel.dart",
      "cases": [
        {"input_hex": "raw frame bytes", "expected_parsed": {"type": "data", "payload": "..."},
         "description": "parse a DATA frame"}
      ]
    },
    {
      "component": "API path builder",
      "source_file": "lib/net/api_builder.dart",
      "cases": [
        {"input": "/tunnel/api/v1/user", "expected": "/go/api/v1/user",
         "description": "path prefix substitution"}
      ]
    }
  ]
}
