...
| 能力 | 探测方式 | 使用插件 | 用途 |
|------|---------|---------|------|
| Playwright | `mcp__playwright__browser_navigate` 或 `mcp__plugin_playwright_playwright__browser_navigate` 可用性（任一可用即就绪） | demo-forge, dev-forge, deadhunt | UI 自动化：验证、E2E 测试、死链扫描 |
