...
**原则：源码中每一个会引起 UI 变化的事件处理函数 = 一次截图。不手工挑选，穷举。**

LLM 读每个页面/组件的源码 → 找出所有绑定了事件处理的 UI 元素（onClick/onSubmit/onChange/onSelect/onToggle/onDrag/...）→ 对每个事件判断"触发后 UI 会怎么变" → 记录为一条 interaction。

**CRUD 全状态**（每个业务实体）：
- Create：空表单 → 填写 → 校验失败截图 → 修正 → 提交成功截图
- Read：列表(有数据) + 详情 + 空状态(无数据)
- Update：编辑表单(预填) → 修改 → 保存
- Delete：确认弹窗 → 删除后列表更新（用刚创建的测试数据，不破坏系统）

**所有其他 UI 变化触发点**（LLM 从事件绑定穷举）：
- 筛选/排序/搜索 → 列表内容变化
- 分页 → 数据切换
- Tab 切换 → 面板内容变化
- 展开/折叠 → 区域显隐
- 视图模式切换（列表/卡片/表格）→ 布局变化
- 下拉框/弹出菜单 → 浮层出现
- 表单校验 → 错误提示出现
- 权限不足操作 → 提示出现
- 通知 → toast/snackbar 出现
- 任何其他 LLM 在源码中发现的 UI 变化触发点

**删除安全**：用 Create 刚创建的数据做 Delete（自产自销）。无法安全删除 → 只截确认弹窗，标记 `DELETE_SKIPPED_SAFETY`。

**大量交互（100+）的执行优化**：
- 按模块分组 → 不同模块的交互用 Agent 并行采集（互不干扰的页面可以并行截图）
- 同一页面的多个交互串行执行（避免状态冲突）
- LLM 自行判断哪些可以并行、哪些必须串行
