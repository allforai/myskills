#!/usr/bin/env python3
"""Transactional candidate admission for Grillstorm integration refs.
...
def admit_candidate(
    *, repo, integration_ref, expected_integration_commit, task_commit, task_id,
    ...
    candidate_ref = f"refs/grillstorm/candidates/{task_component}/{transaction_component}"
    worktree_root = Path(tempfile.mkdtemp(prefix=f"grillstorm-candidate-{task_component}-"))
...
def recover_intent(*, repo, integration_ref, intent, event_writer, event_exists,
    ...
    if (not worktree.is_absolute() or worktree.name != "worktree" or
            not worktree.parent.name.startswith("grillstorm-candidate-")):
