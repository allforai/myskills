# 责任域-2 断言检查
## 负责 capabilities：feature-gap、ui-design

读取以下文件：
- `/Users/aa/workspace/myskills/claude/meta-skill/knowledge/capabilities/ui-design.md`

注意：feature-gap 可能没有独立的 capability `.md` 文件，若不存在，从 product-analysis.md 与 ui-design.md 推断其契约，并在 b2_findings 中注明。

## Contract 表

| capability | artifact_path | required_fields | validation_mode |
|------------|---------------|-----------------|-----------------|
| feature-gap | `.allforai/feature-gap/gap-tasks.json` | gaps[].task_ref, gaps[].type | B1 mechanical |
...
**断言 B2（语义链路）**：
- ui-design：`.md` 是否要求 screens 覆盖用户确认范围内的任务，且不在范围内的任务不出现界面？
...
```yaml
domain: 域-2
capabilities: [feature-gap, ui-design]
