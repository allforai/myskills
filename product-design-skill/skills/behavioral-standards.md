...
description: >
  ...
  product-design Phase 3.6. Scans screen-map to detect cross-screen behavioral inconsistencies
...
> 扫描 screen-map → 检测跨界面行为不一致 → 用户确认 → 输出 behavioral-standards.json（含界面标签映射）+ behavioral-standards-report.md

## 目标

在 UI 设计开始前识别产品中跨界面的行为不一致，通过统一标准保证"同类交互同种体验"：

1. **扫描** — 读取 screen-map.json，检测 9 类行为不一致
...
Phase 3.5: design-pattern → pattern-catalog.json    ← 可选输入（增强分析）
Phase 3.6: behavioral-standards → behavioral-standards.json ← 本技能
Phase 4-7: 并行组（ui-design 消费 behavioral-standards）    ← 下游
Phase 8: design-audit → Behavioral Consistency 维度        ← 事后
```
...
Preflight: 检查前置文件存在性
    ↓ 自动
Step 1: 行为扫描（扫描 screen-map，分类 9 类行为方案）
    ↓ 自动
...
> 目标：检测 screen-map.json 中跨界面的 9 类行为不一致。
...
不修改上游文件（screen-map.json）。下游技能通过 behavioral-standards.json 中的 `screen_behavioral_tags` 查找行为标准。
...
行为标准数据写入独立文件 behavioral-standards.json（含 `screen_behavioral_tags` 映射），不修改 screen-map.json。
