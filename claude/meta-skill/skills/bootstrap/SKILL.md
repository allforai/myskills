  ...
  "nodes": [
    {
      ...
      "source_inputs": ["<project-relative product source files, directories or globs this node READS to produce its own documents and evidence; never the source a node depending on this one writes; explicit [] only when no product source is relevant>"],
      "parallel_write_scopes": ["<project-relative paths this node writes; required when the node writes product source>"],
...
- `source_inputs`: the product source (files, directories or globs, project-relative) this node **reads** to produce its documents, contract and evidence. Required on every node that consumes `requirement_refs`; write explicit `[]` only when no product source is relevant. Omission or a malformed value is refused by all public gates (`missing_source_inputs` / `invalid_source_inputs`) — never a silent skip of freshness. Do not declare the whole repository to silence drift; extra files consumed during execution are registered through the freshness `read` operation. Retained completed legacy nodes keep their historical contract and no provenance is invented for them.
  Declare what the node reads, never what the flow writes after it. A design, spec or concept node that names the implementation tree it governs (`mobile/**`, `src/**`) inverts the graph: building that implementation stales the design it was built from, the design sits behind a freeze that a run may not reopen, and the delivery can never publish. Such a node's source is its upstream input — the brief, the contract, the prior audit — and an audit of pre-existing code belongs to the node that performed it, pinned to what it actually read. Implementation trees belong on the implementation and QA nodes that read or write them. `validate_bootstrap.py` refuses the inversion as `inverted_source_inputs`.
- `parallel_write_scopes`: the project-relative paths this node writes. Required on every node that writes product source, and the only declaration that stops a delivery from invalidating the work that planned it; an undeclared write scope exempts nothing. Mirror it in the Node-spec frontmatter.
