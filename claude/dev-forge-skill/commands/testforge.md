...
```
 ...
 ╱   Component+Unit    ╲   ← 函数、store、service、组件渲染（Path A）
```

> **执行顺序不等于金字塔层级**：金字塔从底到顶是测试粒度，
> 但执行顺序是 Chain 0 → **Path E（操作链，最高 ROI）** → Path A/D/C/B。
> Path E 排在 Unit 之前，因为接缝层 bug 只有 E2E 操作能暴露（铁律 #30）。
...
| **30** | **E2E 操作链优先于 Unit 测试：Path E 排在 Chain 0 之后立即执行，不等 Path A** |
