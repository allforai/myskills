...
所有文档路径相对于本插件根目录（`codex/product-design-skill/`）。
...
| 能力 | 探测方式 | 使用插件 | 用途 |
|------|---------|---------|------|
| Playwright | Playwright MCP `browser_navigate` 可用性（任一可用即就绪） | demo-forge, dev-forge, deadhunt | UI 自动化：验证、E2E 测试、死链扫描 |
| Stitch UI | `Stitch MCP create_project` 可用性 | product-design | 高保真 UI 视觉稿生成（Google Stitch） |
...
| 能力 | 使用插件 | 用途 |
|------|---------|------|
| 网络搜索 | product-design, demo-forge | 搜索驱动设计、媒体搜索降级 |
...
编码助手使用**延迟加载（deferred tools）**机制：MCP 工具可能已注册但未激活到当前上下文。检测工具是否可用需要**同时检查两个来源**：
...
1. **检查 MCP 工具**：在激活工具列表和 `<available-deferred-tools>` 中查找 Playwright MCP `browser_navigate`（任一前缀、任一列表匹配即通过）
   - 可用 → **✅ 就绪**
   - 不可用 → **❌ 未安装**（提示安装 Playwright MCP，见 Step 1.5a）

#### 1b. OpenRouter

1. **MCP 工具通道**：在激活工具列表和 `<available-deferred-tools>` 中查找 `OpenRouter MCP ask_model`
   - 可用 → MCP XV 就绪
   - 不可用 → MCP XV 未就绪（可能需要重启编码助手）
...
1. **MCP 工具通道**：在激活工具列表和 `<available-deferred-tools>` 中查找 `AI Gateway Brave search`
...
1. **MCP 工具通道**：在激活工具列表和 `<available-deferred-tools>` 中查找 `AI Gateway image generation`
...
1. **MCP 工具通道**：在激活工具列表和 `<available-deferred-tools>` 中查找 `AI Gateway FLUX image generation`
   - 可用 → fal.ai MCP 就绪
   - 不可用 → fal.ai MCP 未就绪
...
1. **检查 MCP 工具**：在激活工具列表和 `<available-deferred-tools>` 中查找 Stitch MCP `create_project`（在工具列表中匹配即通过）
   - 可用 → **✅ 就绪**
   - 不可用 → 检查 `~/.stitch-mcp/config/application_default_credentials.json` 是否存在
     - 凭证存在 → 检查是否包含 `quota_project_id` 字段：
       - 有 → **⚠️ OAuth 已完成但 MCP 未加载**（可能需要重启编码助手）
...
| 网络搜索 | 内置 | 就绪 | product-design, demo-forge | 搜索 |
| MCP 服务器 | 构建产物 | {已构建/未构建} | — | AI Gateway（OpenRouter + Google AI + Brave） |

降级链:
  Brave 不可用 → 网络搜索 → AI 生成
...
```bash
MCP 注册命令（参见平台文档）: playwright -- npx -y @playwright/mcp@latest
```

安装成功后提示：

```
Playwright MCP 已注册到用户级配置。需重启编码助手 后生效。
```
...
**Step 4: 确认 MCP 已在插件 MCP 配置中注册**

检查 MCP 配置文件 是否包含 `stitch` 服务器配置：
- 已有 → 跳过
- 缺失 → 添加到 MCP 配置：
```json
"stitch": {
...
**原理**：
- MCP 工具路径：MCP 配置的 env 块使用 `${VAR}` 引用，启动 MCP 服务器时从 shell 环境变量注入
...
内置:
  网络搜索        就绪            product-design, demo-forge — 搜索

{若有未就绪的 MCP 工具或 Key：执行 setup 工作流进行安装和配置}
{若 MCP 已安装但工具不可用：需重启编码助手 加载 MCP 服务器}
...
下一步：重启编码助手 后执行 setup check 验证连接。
```
...
1. **同步插件源码**（关键步骤）：
   ...
   拉取最新的插件源码，确保本地版本是最新的。

   ```bash
   # 拉取最新源码
   cd <plugin-source-dir> && git pull origin main
   ```
...
3. **验证更新**：检查插件版本号是否已更新到最新

5. **MCP 配置保护**：对每个更新的插件，对比更新前后的 MCP 配置：
   - 更新前：读取 `<installPath>/` MCP 配置 保存为 `old_mcp`
   - 更新后：读取新的 MCP 配置 保存为 `new_mcp`
...
⚠️ 插件或 MCP 服务器有更新时，必须重启编码助手 才能生效。
   请退出当前会话并重新启动编码助手。
```
...
- 提示文案必须包含「请退出当前会话并重新启动编码助手」，不能只说"需要重启"

## 铁律（强制执行）

1. **Key 不落项目目录** — API Key 不得写入 `.allforai/` 或任何用户项目目录下的文件
2. **Key 不写入 JSON** — API Key 不写入 MCP 配置或任何 JSON 配置文件。MCP 配置仅使用 `${VAR}` 引用环境变量
