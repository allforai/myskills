...
2. In Step 3 (Generate Node-Specs), include a `setup-runtime-env` node as the **first
   node before any code execution** (before demo-forge, before any service startup)
3. The `setup-runtime-env` node-spec is **project-specific** — LLM generates it based on
   Step 1.1-1.4 analysis (detected databases, caches, AI services, storage, auth, etc.)

**What the generated `setup-runtime-env` node does (at /run time):**
...
**Entry/exit for setup-runtime-env:**
- entry_requires: `[]` (first node, no dependencies)
- exit_requires: `file_exists: .allforai/bootstrap/runtime-env.json`
  (plus `command_succeeds` checks for critical services like DB connectivity)
