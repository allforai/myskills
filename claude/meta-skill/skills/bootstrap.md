...
- **Puzzle-game level content pipeline rule:** When `game_genre` is `match-3`, `tile-match`, `puzzle-casual`, or similar AND `puzzle-design` is selected AND the design spec indicates `level_count > 50`: generate `level-content-pipeline` instead of (or in place of) a generic GUI `level-editor-tool` node. The `level-content-pipeline` node-spec MUST produce: (1) an algorithmic or LLM-driven level generator script, (2) a BFS/A* solver-validator script that rejects unsolvable or trivially easy levels, (3) a generation loop that writes accepted level JSONs to the game client assets. A GUI editor MAY be included as a secondary optional tool for human spot-check, but MUST NOT be the primary level authoring mechanism. See `knowledge/domains/game-genre-specialization.md` §Tile-Match / Match-3 Puzzle for the full specialization pattern.
