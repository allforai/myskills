# Execution Playbook — megastorm pipeline (Grok)
...
escalation surfaces. You (the interactive Grok session) are the orchestration brain;
every headless agent below is a fresh `Grok headless` process.

Spawning an agent: verify the actual Grok executable/wrapper and resolve the complete
effective configuration. Preserve safe model/effort intent, but intersect permissions,
tools, plugins, hooks, MCPs, network, credentials, and writable paths with the Phase 0
deny-by-default capability envelope. Undeclared or unrestrictable behavior fails closed.
Never invoke a bare guessed `grok`, blindly inherit `--yolo`/config, or simulate native
subagents with headless calls. `MEGASTORM_GROK_COMMAND` is a JSON-argv override, not a
verification bypass. Headless output is descriptor-gated NDJSON and must assemble to the
role envelope; terminal prose is never scraped.

## Phase -1 — Preflight
Verify:
- a reliable current Grok host argv (or explicit override), `git`, `python3`; `prompts/`,
  `scripts/` exist beside this playbook.
- Native Grok planning and fresh-context subagents are available for Phase 0 and
  Phase 1.1–1.5.
...
5. **Mint the frozen registry (you are the single owner).** Write into the overview,
   ...
   markers, a ```json object per `schemas/megastorm.md`: `requirements` (`R-<module>-NN` for every
...
6. **Resolve the three model tiers WITH the human** — never automatically:
   ...
   available to this Grok Build install (`Grok headless -m <name>` must work):
...
### 1.1 Design — fan-out
One `Grok headless` (`think` model) per module: `prompts/design-agent.md` + the module spec
+ the frozen registry block. Run them in parallel (shell background jobs) or serially —
collect M design manifests (JSON per `schemas/megastorm.md`).
...
- Then one `Grok headless` (`think`) with `prompts/closure-critic.md` for the prose-level
  judgment, ≤3 rounds. Unresolved → HALT to user.

### 1.3 Plan — fan-out
One `Grok headless` (`think`) per design: `prompts/plan-agent.md` → plan-task array
...
### 1.4 Reverse review
One `Grok headless` (`think`) with `prompts/reverse-critic.md` over all spec/design/plan
...
- **Capability envelope:** `Grok headless` uses workspace-write + ephemeral sessions and a
