...
| **超大** | **> 8** | **> 30** | **建议拆分**：询问用户是否拆成 2-3 个独立产品分批设计 |
...
| 能力 | 探测方式 | 重要性 |
|------|---------|--------|
| OpenRouter (MCP) | `OpenRouter MCP ask_model` 可用性 | 可选 |
...
外部能力:
  ...
  Stitch UI           ✗ 未就绪        UI 视觉稿（可选，执行 `setup check` 查看详情）
...
未配置 API Key 的服务提示执行 `setup` 工作流配置（Key 存储在 shell 环境变量）。
