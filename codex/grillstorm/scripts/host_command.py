...
def resolve_invocation(template=None, environ=None, start_pid=None,
    ...
    if template is not None:
        ...
        return InvocationSpec(exe, (), (), "legacy-template", tuple(tokens), template,
                              verified=False)
    raw = env.get("GRILLSTORM_CODEX_COMMAND")
    if raw is not None:
        try:
            argv = json.loads(raw)
        except ValueError as exc:
            raise HostCommandError("GRILLSTORM_CODEX_COMMAND is not valid JSON") from exc
        contract = wrapper_contract
        if contract is None and env.get("GRILLSTORM_CODEX_WRAPPER_CONTRACT"):
            try:
                contract = json.loads(env["GRILLSTORM_CODEX_WRAPPER_CONTRACT"])
            except ValueError as exc:
