...
- The order is fixed and is the one CLAUDE.md's "Which entry for which situation" table
  ...
  <!-- clause-begin:user-steps-cross-exam-kept -->
- The only exemption is a suppress rule (`suppress-rules.md`): a CLI or library-sdk
  ...
  <!-- clause-end:user-steps-cross-exam-kept -->
...
9. **Experience quality gate (products with UI).** Every specialist contract can pass while the product is still confusing to use. On non-game product routes whose profile records `experience_priority.mode` as `consumer` or `mixed`, plan two critique nodes that run `${CLAUDE_PLUGIN_ROOT}/skills/app-design/40-qa/experience-quality-critique/SKILL.md`: one with `stage: "design"` once the design specs are complete and before any UI implementation node, and one with `stage: "runtime"` after the last UI/product-verify/visual QA. Their `exit_artifacts` carry `.allforai/app-design/qa/experience-quality-critique-design.json` and `.allforai/app-design/qa/experience-quality-critique-runtime.json` (each with its same-name `.html`) plus the human-readable `docs/experience-review/design.md` and `docs/experience-review/runtime.md`. A fresh-context reviewer executes each one, and the node-spec states that the reviewer reads only the artifacts and evidence listed in the Input Contract, never the authoring node's reasoning, transcript or execution notes. Closure and acceptance nodes (`concept-acceptance`, `pipeline-closure-verify`) are `hard_blocked_by` the runtime critique, and a non-empty `must_fix_before_implementation` or `must_fix_before_release` means that artifact has not passed, so `check_artifacts.py` leaves the critique node unfinished. Declare one loop per critique node in `unattended-run-readiness-spec.json.required_repair_loops`: the design loop names the design critique in `qa_node_ids`, a `repair_node_id` that reruns the design sub-skill named by the finding's `repair_route.owner_skill`, and `closure_node_ids` for the nodes waiting on it (`app-design-finalize` under the canonical registry, otherwise the first UI implementation nodes); the runtime loop mirrors its must-fix findings into `experience_gaps`, repairs through an implementation repair node, reruns the affected QA and the runtime critique, and closes on the closure nodes. Both loops take their `max_attempts` from the Must #2 budget; an exhausted budget follows Run Policy and asks nothing during the run. Under the canonical registry the design critique and the design repair node take `capability: app-design` and sit after the spec nodes and before `app-design-finalize`; the runtime critique must not take `app-design` (`quality-checks` is the suggested capability), and no node id contains `cross-exam` or `product-review`. Item 8 states the same law for games, and `validate_bootstrap.py`'s `validate_experience_gate_flow` enforces both. Where `experience_priority.mode` is `none` there is no gate to plan and no direction to confirm: the run excludes `gap-experience-direction` and states `not_applicable.experience`, both with the reason that nobody looks at this product — `freeze` maps that gap question into `exclude` instead of leaving it a dangling pending question, and `plan` carries the `not_applicable` key, which `product_intent.py` accepts only at that mode and otherwise refuses as `experience_not_applicable_on_ui_product`. Suppressing the gate never means going quiet about the question it came from.
