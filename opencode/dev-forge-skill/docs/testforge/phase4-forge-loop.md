...
并行执行 2 个 Agent：
  Agent 1: 加载 `commands/deadhunt.md`
           执行 /deadhunt static（死链 + CRUD 缺口 + 幽灵功能 + 接缝检查）
  Agent 2: 加载 `commands/fieldcheck.md`
...
**跨子项目并行**：同一路径内，不同子项目互相独立，并行执行。例如 Path A 中 website 和 admin 的单元测试可同时锻造。Path B 除外（E2E 链天然跨子项目，按链串行）。
...
3.8 **分页/无限滚动边界测试（Pagination 缺口）**

   > 此步骤专门处理 Phase 1 标记为 `PAGINATION_GAP` 的缺口。
   > 分页是"前端分页请求 ↔ 后端游标/offset"的接缝，加上"用户滚动速度 ↔ API 响应速度"的动态因素 — 典型的接缝+动态 bug 高发区。

   对每个 PAGINATION_GAP 缺口，生成 platform_ui 测试：

   ```
   翻页数据正确性：
     - 加载第 1 页 → 记录所有 item 的唯一标识（ID/名称）
     - 加载第 2 页 → 断言：第 2 页的 item 与第 1 页无交集（不重复）
     - 断言：第 2 页有 ≥1 条数据（非空页）
     - 回到第 1 页 → 断言：数据与最初一致（翻页没破坏缓存）

   并发翻页安全（接缝+动态核心场景）：
     - 用 Playwright 慢网模式 → 快速连续触发 2 次翻页
     - 断言：最终显示的是最后请求的那一页的数据（非中间页）
     - 断言：无数据重复、无数据乱序

   筛选 + 分页交互：
     - 加载第 2 页 → 切换筛选条件 → 断言：自动回到第 1 页
     - 加载第 2 页 → 切换排序 → 断言：自动回到第 1 页
     - 筛选后结果为 0 → 断言：显示 Empty State（非空白/报错）

   边界状态：
     - 滚动/翻页到最后一页 → 断言："加载更多"消失或 disabled 或显示"已全部加载"
     - 只有 1 条数据 → 断言：分页组件隐藏（不显示"第 1 页 / 共 1 页"）
     - 数据总量刚好是 pageSize 的整数倍 → 断言：最后一页后不会多请求一个空页

   加载态防护：
     - 翻页触发后 → 断言：显示 loading 指示
     - loading 中再次触发翻页 → 断言：被防抖/节流阻止（不发重复请求）
     - 验证方式：监听 network 请求数，翻页中快速再翻 → 请求数 ≤ 2（非 N 个）
   ```

   **分页测试的失败分类**：
   - 数据重复（offset 计算错误）→ BIZ_BUG
   - 筛选后未重置分页 → BIZ_BUG
   - 无防抖导致重复请求 → BIZ_BUG
   - 最后一页未正确处理 → BIZ_BUG
   - 缺 loading 态 → BIZ_BUG（UX 接缝问题）
