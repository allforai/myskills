...
- Codex 主控时同一张表映射到 Codex 字面量，见 codex 镜像的 SKILL.md；跨平台双审的外部 reviewer
  按对方平台的判断档字面量调用（`codex exec -m gpt-5.6-sol` / `claude -p --model opus`）。
...
1. **独立 surface 枚举**：派一个 fresh-context agent（`Agent(general-purpose, model: opus)`，判断档），prompt = `$ROOT/knowledge/cross-exam/prompts/census.md` 全文 +
...
- **落账**：entry 带 `journey`、`steps`、`terminal_state`；`journeys[].entry_q` 指到该 entry 的 `q`，
  `status` 改 `examined`。
- **扫全模式下**：所有 `not_examined` 旅程并行扇出，一条旅程一个 fresh-context 实测官（同样 `model: sonnet`），收齐后逐条裁决。
...
- **派实测官**：`Agent(general-purpose, model: sonnet)`（取证档），prompt = `$ROOT/knowledge/cross-exam/prompts/prober.md`
...
- **孤例还是一类（每条 `gap|drift` 落账时必答）**：这个缺口是这一处独有，还是某个结构模式的
  ...
  ② 派**同类位点枚举官**（fresh-context，`model: sonnet` 取证档，只枚举不取证，成本一问一 agent）：prompt =
