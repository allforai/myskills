---
name: cross-exam
description: Package with three explicit reviews. cross-exam audits completion and rule consistency; product-review critiques product value; keep-code-simple reviews implementation simplicity, advice only. None is automatic. When the user names product-review, follow product-review.md; for keep-code-simple, follow keep-code-simple/SKILL.md. Neither route runs the completion protocol.
---
...
读取 `$ROOT/rule-consistency.md`，按其中的来源快照 → 独立规则审查官 → 用户澄清
流程执行；固定 prompt 为 `$ROOT/prompts/rules.md`。它比较规则与规则，不替代实测。
