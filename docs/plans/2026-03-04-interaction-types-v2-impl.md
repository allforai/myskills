...
## Task 13: 写行为原语索引（interaction-types.md 新章节）

**文件：**
- 修改：`product-design-skill/docs/interaction-types.md`（在"各阶段使用指南"之前插入新章节）
...
> 交互类型之间存在共享的底层行为单元。**凡共享同一原语的类型，前端实现可共享组件或逻辑。**
> design-to-spec 阶段在生成逐页规格之前，先按此索引识别项目内的共享组件边界。

| 原语 | 描述 | 使用该原语的类型 |
|------|------|----------------|
| `VirtualList` | 大数据量列表，只渲染可视区域 | MG1, MG2-L, CT7 |
| `InfiniteScroll` | 滚动到底部自动加载下一页 | CT1, CT2（评论）, WK1（历史消息） |
| `PullToRefresh` | 下拉触发数据刷新 | CT1, MG1, RT4（移动端） |
| `SwipeAction` | 列表行侧滑显示操作按钮 | MG1, WK1, RT4（移动端） |
| `HorizontalSwipe` | 横向滑动做二选一决策 | CT4 |
| `VerticalSwipe` | 纵向滑动切换至下一条内容 | CT8 |
| `DragAndDrop` | 跨容器拖拽，触发位置/状态变更 | WK5, WK6, WK7 |
| `CanvasDrag` | 自由坐标系内拖拽节点/视口 | WK4, EC1（图片缩放）, CT6 |
...
1. 汇总 screen-map.json 中项目出现的所有 `interaction_type`
2. 查本索引，找出被多个类型共用的原语
3. 将这些原语列入「共享组件规划」，统一封装后供各页面复用
4. 再进行逐页技术规格生成

**示例：** 项目含 CT1 + MG1 + MG2-L + RT4
- 共用 `VirtualList` → 封装 `<DataList>` 共享组件
- CT1 + RT4 共用 `AppendOnlyStream` → 封装 `<StreamList>` 共享组件
- MG1 + WK1 + RT4 共用 `SwipeAction`（移动端）→ 共用侧滑手势逻辑
```

**验证：** 章节存在，原语表完整（18 行），使用方式说明包含示例。

**提交：**
```bash
git add product-design-skill/docs/interaction-types.md
git commit -m "docs(interaction-types): 新增行为原语索引章节（18 种原语 + 类型映射）"
```

---

## Task 14: 更新 design-to-spec.md — 新增原语识别阶段

**文件：**
- 修改：`dev-forge-skill/skills/design-to-spec.md`

**新增内容位置：** 在 Step 2（按 interaction_type 生成页面规格）之前，插入新的 Step 1.5 或重编为 Step 2（原 Step 2 顺延）。
...
2. 查 `docs/interaction-types.md` 的「行为原语索引」，找出被 ≥2 个界面共用的原语
3. 按原语分组，输出共享组件规划表：
...
**验证：** design-to-spec.md 中原语识别步骤存在，YAGNI 约束明确（≥2 次才封装）。

**提交：**
```bash
git add dev-forge-skill/skills/design-to-spec.md
git commit -m "feat(design-to-spec): 新增原语识别阶段，在逐页规格前规划共享组件边界"
```
