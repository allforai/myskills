...
def resolve_invocation(template=None, environ=None, start_pid=None,
    ...
    if template is not None:
        ...
        return InvocationSpec(exe, (), (), "legacy-template", tuple(tokens), template,
                              verified=False)
    # MEGASTORM_* is the pre-rename spelling; keep reading it so existing shells work.
    raw = env.get("SUPERSTORM_CODEX_COMMAND", env.get("MEGASTORM_CODEX_COMMAND"))
    if raw is not None:
        try:
            argv = json.loads(raw)
        except ValueError as exc:
            raise HostCommandError("SUPERSTORM_CODEX_COMMAND is not valid JSON") from exc
        contract = wrapper_contract
        raw_contract = env.get("SUPERSTORM_CODEX_WRAPPER_CONTRACT", env.get("MEGASTORM_CODEX_WRAPPER_CONTRACT"))
        if contract is None and raw_contract:
            try:
