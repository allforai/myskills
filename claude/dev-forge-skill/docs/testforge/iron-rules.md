...
E2E 操作链（Path E）排在 Chain 0 之后**立即执行**，不等 unit 测试（Path A）完成。

**原因**：
- 实战数据：400 个 unit 测试全绿 + 0 个真实 bug 发现；1 条 E2E 操作链 5 分钟 + 发现"删除有关联数据时前端报错"的接缝 bug
- E2E 操作链测的是**用户真实操作路径**（点击按钮 → API 调用 → 错误处理 → toast 提示），unit 测试测的是**代码逻辑**
- 两者发现的 bug 集合完全不重叠 — 不存在"先 unit 后 E2E"的依赖关系
- 先发现接缝 bug 再补 unit 测试 = 高效；先补 unit 测试再发现接缝 bug = 浪费

**执行方式**：Path E 和 Path A 可并行执行。Path E 发现的 BIZ_BUG 立即修复，不进入 CG-1 轮次计数。
