...
| 步骤 | 标准模式 | 全自动模式 |
|------|----------|-----------|
| **Step 0 竞品提问** | ask the user竞品 | 读 `pipeline_preferences.competitors`：非空 → 直接填充 `competitor-profile.json`，跳过提问；空 → 记录 `competitors: []`，Step 9 Part 3 跳过竞品差异 |
| **Step 0 画像确认** | confirm with user | 自动确认，记入 `pipeline-decisions.json`（`decision: "auto_confirmed"`） |
| **Step 1 角色确认** | confirm with user | 自动确认 |
| **Step 2 任务确认** | confirm with user | 自动确认 |
| **Step 3 业务流确认** | confirm with user | 自动确认 |
| **Step 4 冲突确认** | confirm with user | 自动确认（冲突记录到 decisions.json） |
| **Step 5 约束确认** | confirm with user | 自动确认 |
| **Step 6.5 体验基因** | confirm with user | 自动确认（DIFF 数 = 0 → 停下来问用户） |
| **Step 7 数据建模** | confirm with user | 自动确认 |
| **Step 8 视图对象** | confirm with user | 自动确认 |
| **Step 9 校验** | confirm with user | ERROR 级问题（必填字段缺失、引用断裂）→ **停下来问用户**；WARNING/INFO → 记日志自动继续 |
...
**加载方式**：使用 Read 工具读取 `./docs/product-map/<filename>` 获取当前阶段的完整步骤定义。每个子文件是自包含的，包含该阶段所有步骤的完整 schema、示例、审计规则和输出规范。
