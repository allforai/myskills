...
Step 4.3: Tasks 验证循环（4D/6V+V9/XV/闭环 — 由 Auditor Agent 执行）
  ...
  **小循环 A: 4D/6V+V9+V10 任务审计**
  ...
  | **V13 Attention Split** | **注意力分离完整度**（元门禁）：Auditor 对每个 HIGH risk 的 B2/B3 主任务 + 随机 20% 其他任务，列出关注维度并验证：(1) 维度 > 3 时是否有子任务？(2) 每个 B2 _Acceptance_ 条件是否有对应的 B5 测试断言？ | `TASK_ATTENTION_SPLIT_MISSING` / `TASK_TEST_DERIVATION_MISSING` |

  **V9 覆盖率、V10 溯源、V11 验收条件、V12 体验差异化、V13 注意力分离是强制审计维度**，CRITICAL 级遗漏必须修复后才能退出循环。
  ...
  **修正方式**：
  ...
  - `TASK_ATTENTION_SPLIT_MISSING`（HIGH risk 任务）→ CRITICAL — Auditor 按元规则为该任务拆出缺失的子任务
  - `TASK_ATTENTION_SPLIT_MISSING`（非 HIGH risk）→ WARNING — 记录但不阻塞
  - `TASK_TEST_DERIVATION_MISSING`（HIGH risk B2）→ CRITICAL — 从 _Acceptance_ 生成缺失的 B5 测试任务
  - `TASK_TEST_DERIVATION_MISSING`（非 HIGH risk）→ WARNING — 记录
  ...
  **退出条件**：
  ...
  - V13 ATTENTION_SPLIT CRITICAL = 0（HIGH risk 任务的注意力分离已完成）
  - V13 TEST_DERIVATION CRITICAL = 0（HIGH risk B2 的 Acceptance 均有测试覆盖）

  **大循环 3 轮后仍有问题** → 记录为已知问题到 `pipeline-decisions.json`，输出警告，继续（不停）

  → 输出进度: 「Step 4.3 验证 ✓ V9:{X}% V10:{Y}% V11:{Z}% V12 DNA:{W}% V13 分离:{S}% 测试派生:{T}% | gaps:{N} fixed | XV:{status}」
  ↓
