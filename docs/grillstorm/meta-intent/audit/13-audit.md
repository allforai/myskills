...
| 11 | Claude 与 Codex 的实际入口和生成产物消费同一权威规则；脚本测试不冒充真实宿主测试。 | artefact: `codex/meta-skill/scripts` is a symlink to `../../claude/meta-skill/scripts` (one canonical gate-CLI source for both hosts); `claude/meta-skill/scripts/orchestrator/check_artifacts.py` and `codex/meta-skill/knowledge/flow-template.py` both list `accepted_with_gaps`/`accepted_with_warnings` in their `BLOCKING_STATUS_VALUES` (git log: `c77323da fix(meta-skill): align document checks and Codex status reporting (#13)` closed the earlier drift noted in `docs/grillstorm/meta-intent/replan-2/T13-spec-review` §Partial 1); `test_delivery_closure.py::test_accepted_with_gaps_is_a_qualified_run_outcome_not_completion` (host-parametrized, row 9); `codex/meta-skill/test_flow.py::test_accepted_with_gaps_is_not_reported_as_a_ready_artifact`. This is scripted evidence (both hosts' copied CLIs and generated templates), not a real Claude/Codex host dialogue — that distinction is exactly what this row's second clause asks the audit itself to preserve, and #15–#18 (out of scope, see Global Constraints) are where real host proof for this rule would be recorded. | PASS (2 passed in 0.75s); PASS (1 passed in 0.12s) |
| 12 | 基于同步后的当前候选重新验证；历史通过报告仅供回归参考，不直接证明新版本完成。 | `test_delivery_closure.py::test_document_check_that_changes_inputs_cannot_publish_proof_for_the_old_state` (same run as row 7: a prior passing acceptance run cannot be republished as proof once the observed source has moved; the record is rejected `stale` and nothing is published for either state) | PASS (4 passed in 1.76s) |
