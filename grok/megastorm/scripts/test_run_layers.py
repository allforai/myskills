# Grok Megastorm deterministic runner tests.
import json
import pathlib
import subprocess
import sys
...
from run_layers import (ANTI_VACUOUS, HEAVY_CMD_RE, ActiveProcessGroups,
                        build_supervisor_prompt, GrokRunner, EventLog,
                        InfrastructureFailure, atomic_write_json, parse_verdict,
    ...
    def test_infrastructure_retry_separate_from_business_retry(self):
        class Flaky(FakeRunner):
            ...
            def run(self, prompt, model, cwd, role):
                ...
                return super().run(prompt, model, cwd, role)
...
class TestDurabilityAndScope(unittest.TestCase):
    def test_real_execution_refuses_fake_only_stream_descriptor(self):
        args = ["run_layers.py", "orch.json", "tasks.json", "--models", "models.json",
                "--prompts", str(PROMPTS), "--effective-config", "config.json"]
    ...
    def test_agent_environment_excludes_ambient_secret(self):
        ...
        try:
            from host_command import InvocationSpec, ProbeResult
            invocation = InvocationSpec(("/bin/echo",), (), (), "test", ("grok",),
                                        ProbeResult(True, "official Grok Build", "test",
                                                    ("version", "help", "inspect-json",
                                                     "plugin-validate", "streaming-json")))
            self.assertNotIn("MEGASTORM_TEST_SECRET", GrokRunner(invocation=invocation).env)
            self.assertEqual(GrokRunner(invocation=invocation,
    ...
    def test_confirmation_marker_subject_is_never_authoritative(self):
        with tempfile.TemporaryDirectory() as root:
            for cmd in (["git", "init", "-q"], ["git", "config", "user.email", "t@t"],
                        ["git", "config", "user.name", "t"],
                        ["git", "commit", "-q", "--allow-empty", "-m", "init"]):
            ...
            self.assertEqual(recover_confirmed_from_git(
                pathlib.Path(root), baseline, {"T1": {}, "T2": {}}), {})

    def test_event_terminal_recovers_snapshot_gap(self):
        with tempfile.TemporaryDirectory() as td:
            path = pathlib.Path(td, "events.jsonl")
            first = {"run_id": "r", "seq": 1, "event_id": "r:1",
                     "type": "task_terminal",
                     "payload": {"task_id": "T1", "status": "done"}}
            path.write_text(json.dumps(first) + "\n")
            log = EventLog(path, "r", 0)
            self.assertEqual(log.recovered_completed, {"T1": "done"})
            self.assertEqual(log.seq, 1)

    def test_event_log_rejects_forged_sequence(self):
        with tempfile.TemporaryDirectory() as td:
            path = pathlib.Path(td, "events.jsonl")
            path.write_text(json.dumps({"run_id": "r", "seq": 9,
                                        "event_id": "r:9", "type": "x",
                                        "payload": {}}) + "\n")
            with self.assertRaisesRegex(ValueError, "continuity"):
                EventLog(path, "r")

    def test_active_process_groups_terminate_running_child(self):
        registry = ActiveProcessGroups()
...
class TestCensusArtifact(unittest.TestCase):
    def test_exhaustive_census_is_fingerprinted(self):
        with tempfile.TemporaryDirectory() as td:
            root = pathlib.Path(td)
            (root / "clean.txt").write_text("inspected")
            census = root / "census.json"
            census.write_text(json.dumps({"population": [
                {"id": "one", "disposition": "task", "task_id": "T1"},
                {"id": "two", "disposition": "verified-clean",
                 "evidence": "clean.txt"}]}))
            result = validate_census_artifact(census, root, {"T1": {}})
            self.assertEqual(result["claim"], "census-backed")
            self.assertEqual(result["population"], 2)
            self.assertEqual(len(result["sha256"]), 64)

    def test_missing_uncontained_or_unresolved_census_fails(self):
        with tempfile.TemporaryDirectory() as td, tempfile.TemporaryDirectory() as outside:
            root = pathlib.Path(td)
            with self.assertRaises(ValueError):
                validate_census_artifact(root / "missing.json", root, {})
            escaped = pathlib.Path(outside, "census.json")
            escaped.write_text('{"population":[{"id":"x","disposition":"task","task_id":"T1"}]}')
            with self.assertRaisesRegex(ValueError, "contained"):
                validate_census_artifact(escaped, root, {"T1": {}})
            bad = root / "bad.json"
            bad.write_text('{"population":[{"id":"x","disposition":"unknown"}]}')
            with self.assertRaisesRegex(ValueError, "unresolved"):
                validate_census_artifact(bad, root, {})

    def test_census_claim_requires_every_task_terminal_disposition(self):
        metadata = {"method": "census", "claim": "census-backed",
                    "members": [
                        {"id": "m-done", "disposition": "task", "task_id": "done"},
                        {"id": "m-reality", "disposition": "task", "task_id": "rg"},
                        {"id": "m-skip", "disposition": "task", "task_id": "skip"},
                        {"id": "m-escalate", "disposition": "task", "task_id": "bad"},
                        {"id": "m-missing", "disposition": "task", "task_id": "missing"},
                        {"id": "m-clean", "disposition": "verified-clean",
                         "evidence": "proof.txt"}]}
        result = reconcile_census(
            metadata,
            [{"task_id": "done", "status": "done"},
             {"task_id": "rg", "status": "reality_gated"},
             {"task_id": "bad", "status": "escalate"}],
            {"skip": "failed-parent"})
        self.assertEqual(result["claim"], "Completeness unverified")
        self.assertEqual(result["unresolved_members"], [
            {"member_id": "m-skip", "task_id": "skip", "status": "skipped",
             "blocked_by": "failed-parent"},
            {"member_id": "m-escalate", "task_id": "bad", "status": "escalate",
             "blocked_by": None},
            {"member_id": "m-missing", "task_id": "missing", "status": "unresolved",
             "blocked_by": None}])
        self.assertEqual(result["terminal_dispositions"],
                         {"verified": 1, "reality_gated": 1, "verified-clean": 1})

    def test_census_claim_survives_only_fully_reconciled_population(self):
        metadata = {"method": "census", "claim": "census-backed",
                    "members": [{"id": "m", "disposition": "task", "task_id": "T1"}]}
        result = reconcile_census(metadata, [{"task_id": "T1", "status": "done"}], {})
        self.assertEqual(result["claim"], "census-backed")
        self.assertEqual(result["unresolved_members"], [])
    ...
    def test_main_publishes_integration_ref_without_touching_user_tree(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            (base / "effective-config.json").write_text("{}")
            fake = base / "grok"
            fake.write_text(
                "#!/usr/bin/env python3\n"
                "import json,pathlib,sys\n"
                "a=sys.argv[1:]; cwd=a[a.index('--cwd')+1]; prompt=a[a.index('-p')+1]\n"
                "if 'anti-fake-completion verifier' in prompt:\n"
                " envelope={'kind':'supervisor','verdict':'confirmed','summary':'ok'}\n"
                "else:\n"
                " pathlib.Path(cwd,'out.txt').write_text('done')\n"
                " envelope={'kind':'executor','status':'complete','summary':'implemented','touched_paths':['out.txt']}\n"
                "payload=json.dumps(envelope)\n"
                "print(json.dumps({'type':'assistant.chunk','id':'c1','sequence':0,'text':payload}))\n"
                "print(json.dumps({'type':'stream.terminal','id':'t1','status':'success'}))\n")
            fake.chmod(0o755)
            args = ["run_layers.py", str(base / "orch.json"), str(base / "tasks.json"),
                    ...
                    "--effective-config", str(base / "effective-config.json")]
            from host_command import ProbeResult
            probe = lambda _launcher: ProbeResult(
                True, "official Grok Build test double", "fake-test/1",
                ("version", "help", "inspect-json", "plugin-validate", "streaming-json"))
            env = {"MEGASTORM_GROK_COMMAND": json.dumps([str(fake)])}
            self.assertEqual(main(args, identity_probe=probe, environ=env,
                                  allow_nonconformant_descriptor=True), 0)
...
class NoCommitFakeRunner(FakeRunner):
    ...
    def run(self, prompt, model, cwd, role):
        ...
        return FakeRunner.run(self, prompt, model, cwd, role)
...
class TestMainTreeConsistency(unittest.TestCase):
    ...
    def test_merge_failure_reason_carries_stderr(self):
        ...
        with tempfile.TemporaryDirectory() as root:
            ...
            orig_run = runner.run
            def run_and_diverge(prompt, model, cwd, role):
                out = orig_run(prompt, model, cwd, role)
                if "anti-fake-completion verifier" in prompt:
