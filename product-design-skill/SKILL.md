...
name: product-design
description: >
  Product design suite: product-concept (产品概念), review (统一审核站点, 6 tabs), market-validate (市场验证),
...
流程：concept → **review（概念 tab）** → **market-validate** → product-map → **review（地图 tab）** → journey-emotion → experience-map → interaction-gate → **review（线框+数据模型 tab）** → **Stitch 决策点** → [use-case ∥ feature-gap ∥ ui-design] → **review（UI tab）** → design-audit，每阶段间插入检查点验证产出完整性。
...
推荐流程：[product-concept → review(概念) → market-validate →] product-map → review(地图) → journey-emotion → experience-map → review(线框+数据模型) → [use-case ∥ feature-gap ∥ ui-design] → review(UI) → design-audit
